...
<body>
      ...
      list.innerHTML = rows.length
        ? `<table class="dcs-table"><thead><tr><th>When</th><th>Type</th><th>Verified</th><th>What happened</th><th></th></tr></thead><tbody>`
          + rows.map((e, i) => {
              ...
              const hdrs = det.headers ? Object.entries(det.headers).map(([k, v]) => `${k}: ${v}`).join('\n') : '';
              const cands = Array.isArray(det.computed) ? det.computed
                : (det.computed ? [{ scheme: 'HMAC(body)', value: det.computed }] : []);
              const candHtml = cands.map(c => {
                const m = det.sign && c.value && det.sign.toLowerCase() === c.value.toLowerCase();
                return `<div style="font-size:11px;margin:.3rem 0"><b>${esc(c.scheme)}:</b> <code>${esc(c.value)}</code> ${m ? '<span style="color:#16a34a">✓ match</span>' : '<span style="color:#dc2626">✗ differs</span>'}</div>`;
              }).join('');
  ...
  function openZortForm(store) {
    ...
    if (zfRec) zfRec.value = (store?.recordOnlyClients || []).join(', ');
  }
