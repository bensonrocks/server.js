...
async function fetchLabelPdf(store, { id, number, tracking, labelUrl, skipFileEndpoint } = {}) {
  ...
  for (const r of (rows || [])) {
    ...
    const lc = {}; for (const k of Object.keys(r)) if (typeof r[k] === 'string' || typeof r[k] === 'number') lc[k.toLowerCase()] = String(r[k]);
    const fmt = String(lc.format ?? lc.formattype ?? '').toLowerCase();
    offered.push(`${String(lc.type || 'label')}/${fmt || 'no-format'}`);
    const strFields = Object.values(lc);
    const b64Cands = [...new Set([lc.data, lc.file, lc.filedata, lc.pdf, lc.pdfdata,
      ...strFields.filter(s => s.length > 500 && !/^https?:/i.test(s))].filter(Boolean))];
    for (const cand of b64Cands) {
      try { const buf = _asPdf(Buffer.from(cand, 'base64')); if (buf) return { pdf: buf, via: 'list-data' }; } catch (_) {}
    }
    const urlCands = [...new Set([lc.linkurl, lc.link, lc.url, lc.fileurl, lc.pdfurl, lc.labelurl,
      (fmt === 'url' ? lc.data : ''), ...strFields]
      .map(s => String(s || '').trim()).filter(s => /^https?:\/\//i.test(s)))];
    for (const url of urlCands) {
      try {
        // The store's own credentials go ONLY to the store's own host — a
        // signed carrier URL must never receive them.
        const r2 = await fetch(url, {
          headers: _sameHost(url, storeBase) ? authHeaders(store) : undefined,
          signal: AbortSignal.timeout(30000),
        });
        if (r2.ok) {
          const buf = _asPdf(Buffer.from(await r2.arrayBuffer()));
          if (buf) return { pdf: buf, via: 'list-url' };
        }
  ...
  if (!rows || !rows.length) {
    ...
    return { pdf: null, why: 'empty', fileTried, fileWorked: false,
             detail: listError || 'the channel has not generated it yet' };
  }
  const rowKeys = [...new Set(rows.flatMap(r => Object.keys(r)))].slice(0, 15);
  return {
    pdf: null, why: 'unusable', offered, keysSeen: rowKeys, fileTried, fileWorked: false,
