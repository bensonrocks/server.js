...
async function fetchLabelPdf(store, { id, number, tracking, labelUrl, skipFileEndpoint } = {}) {
          ...
          if (/html/i.test(d.type) && raw.length < 1024 * 1024) {
            ...
            for (const hop of hops) {
              try {
                ...
                if (pdf3) { d.hop = hop.slice(0, 120); tried.push(d); return { pdf: pdf3, via: `${via}-url-html` }; }
              } catch (_) {}
            }
            // A page with no label-ish link that talks about signing in is a
            // page for a BROWSER — the honest dead end, said in words.
  ...
  return {
    pdf: null, why: 'unusable', offered, keysSeen: rowKeys, tried, probe, fileTried, fileWorked: false,
