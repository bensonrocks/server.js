...
async function fetchLabelPdf(store, { id, number, tracking, labelUrl, skipFileEndpoint } = {}) {
  ...
  } catch (e) { listError = e.message; console.warn('[zort-label] list endpoint:', e.message); rows = []; }
  for (const r of rows) {
    const fmt = String(r.Format || r.format || '').toLowerCase();
    const data = r.Data || r.data || '';
    offered.push(`${String(r.type || r.Type || 'label')}/${fmt || 'no-format'}`);
  ...
  return {
    pdf: null, why: 'unusable', offered, fileTried, fileWorked: false,
    detail: `the channel offered ${offered.join(', ')} — nothing we can import as a PDF`
