...
async function fetchLabelPdf(store, { id, number, tracking, labelUrl } = {}) {
  ...
  } catch (e) { console.warn('[zort-label] list endpoint:', e.message); rows = []; }
  for (const r of rows) {
  ...
  if (Number(store.labelFileFails || 0) >= 3) return null;
  try {
    const direct = await getShippingLabel(store, { id, number, tracking });
    if (direct) { store.labelFileFails = 0; return { pdf: direct, via: 'file' }; }
    store.labelFileFails = Number(store.labelFileFails || 0) + 1;
  } catch (e) {
    store.labelFileFails = Number(store.labelFileFails || 0) + 1;
