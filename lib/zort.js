...
const _labelCrypto = require('crypto');
async function fetchLabelPdf(store, { id, number, tracking, labelUrl, skipFileEndpoint } = {}) {
  ...
  // THE PRINT-PAGE LINK, IN FULL, for the browser worker. `tried` records
  // only the host and outcome of each link (it is shown on screen and kept on
  // the outbox entry, and a label URL carries the customer's address) — which
  // is exactly why the worker found no link to open: it read `tried[].url`,
  // a field the diagnostics rewrite had dropped, and skipped itself in
  // silence on the one shape it exists for. Reported from the floor as "zort
  // has labels. why i cant pull?". Carried separately, never on the trail.
  let pageLink = '';
  let listError = '';
  try {
  ...
  // EVERY PARCEL, NOT JUST THE FIRST. A split order carries one label row per
  // box; returning on the first row handed the packer one label for two
  // parcels (reported live on 171067267872131). The first usable row is the
  // answer as before; every FURTHER row that yields a different PDF rides
  // along as `more[]`, and the caller imports each for the same order — the
  // label pipeline files it as another parcel when its tracking number
  // differs, or as a duplicate when it is the same label listed twice.
  let first = null;
  const seen = new Set();
  const more = [];
  for (const r of (rows || [])) {
    ...
    if (!hit) continue;
    const sig = _labelCrypto.createHash('sha256').update(hit.pdf).digest('hex');
    if (seen.has(sig)) continue;
    seen.add(sig);
    if (!first) first = hit; else more.push({ pdf: hit.pdf, via: hit.via });
  }
  if (first) { if (more.length) first.more = more; return first; }
  ...
  return {
    pdf: null, why: 'unusable', offered, keysSeen: rowKeys, tried, probe, pageLink, filesSeen: fileNames, fileTried, fileWorked: false,
