...
const DEFAULT_ADMIN_PW = process.env.ADMIN_PASSWORD || 'Admin1234';
db.prepare("INSERT OR IGNORE INTO staff_users (username, password_hash, role) VALUES (?, ?, 'master')").run('administrator', _sha256(DEFAULT_ADMIN_PW));
db.prepare("UPDATE staff_users SET password_hash = ?, role = 'master' WHERE username = 'administrator'").run(_sha256(DEFAULT_ADMIN_PW));
console.log(`[auth] administrator login ready (master role) — password: ${DEFAULT_ADMIN_PW}`);
