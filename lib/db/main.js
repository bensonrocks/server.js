...
// Ensure administrator always has admin role
db.prepare("UPDATE staff_users SET role = 'admin' WHERE username = 'administrator'").run();
// If ADMIN_PASSWORD env var is explicitly set, always force-update the hash on startup
if (process.env.ADMIN_PASSWORD) {
  db.prepare("UPDATE staff_users SET password_hash = ? WHERE username = 'administrator'")
    .run(_sha256(process.env.ADMIN_PASSWORD));
}
