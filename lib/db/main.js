...
// Seed default staff admin — no password required
const { createHash } = require('crypto');
const _sha256 = s => createHash('sha256').update(s).digest('hex');
db.prepare("INSERT OR IGNORE INTO staff_users (username, password_hash) VALUES (?, ?)").run('administrator', _sha256(''));
db.prepare("UPDATE staff_users SET password_hash = ? WHERE username = 'administrator'").run(_sha256(''));
