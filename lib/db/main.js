...
// Seed default staff admin (INSERT OR IGNORE — never overwrites a changed password)
const { createHash } = require('crypto');
const _sha256 = s => createHash('sha256').update(s).digest('hex');
db.prepare("INSERT OR IGNORE INTO staff_users (username, password_hash) VALUES (?, ?)").run('administrator', _sha256('123'));
