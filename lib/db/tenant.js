...
function getTenantDb(tenantId) {
  ...
  // Migration: add branding columns to client_users for pre-existing tenant DBs
  const clientUserCols = db.prepare("PRAGMA table_info(client_users)").all().map(c => c.name);
  if (!clientUserCols.includes('logo_url'))  db.exec("ALTER TABLE client_users ADD COLUMN logo_url TEXT NOT NULL DEFAULT ''");
  if (!clientUserCols.includes('show_logo')) db.exec("ALTER TABLE client_users ADD COLUMN show_logo INTEGER NOT NULL DEFAULT 0");
