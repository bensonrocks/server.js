...
async function ensureSeed(ownerOrgId) {
  ...
  // Owner (Master) login. Defaults to Master / 12345 for first-run self-use;
  // OVERRIDE both in production via HUNTER_STAFF_EMAIL and HUNTER_STAFF_PASSWORD.
  const email    = process.env.HUNTER_STAFF_EMAIL || 'Master';
  const password = process.env.HUNTER_STAFF_PASSWORD || '12345';
  if (!process.env.HUNTER_STAFF_PASSWORD) {
    console.warn('WARNING: HUNTER_STAFF_PASSWORD not set — seeded Master account uses the default weak password "12345". Set HUNTER_STAFF_PASSWORD (and HUNTER_STAFF_EMAIL) in env vars for production.');
  }
