...
async function runForOrg(org) {
  ...
  try {
    ...
    if (!candidates.length) {
      // Self-diagnose: is web search actually working, or just a thin day?
      set('running', { step: 'No leads — checking why…', written: 0 });
      const probe = await probeWebSearch(client);
      const note = probe.ok
        ? 'Web search is working, but the Scout found no fresh, credible signals matching your wishlist right now. Broaden your market/industries/signals in the wishlist above, raise leads/day, or try again later.'
        : `No leads because web search is not working: ${probe.error} Until web search is enabled for your Anthropic org, the Scout cannot find companies.`;
