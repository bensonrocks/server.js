...
// Auto-enrich ONE decision-maker for an Apollo company: find a priority-title
// person at the org, then reveal their name + email (+ phone when returned).
// Costs Apollo credits (people search + 1 per reveal) — the operator opted in.
// Degrades to a company-phone-only contact, then {}, on any failure.
async function apolloEnrichContact(orgId, market, companyPhone) {
  const fallback = companyPhone ? { phone: companyPhone, source: 'apollo' } : {};
  if (!process.env.APOLLO_API_KEY || !orgId) return fallback;
  let people = [];
  try {
    const s = await apolloPost('mixed_people/search', {
      organization_ids: [orgId], person_titles: APOLLO_TITLES, per_page: 10,
      ...(market ? { person_locations: [market] } : {}),
    });
    people = (s && s.people) || [];
...
async function runForOrg(org) {
  ...
  try {
    ...
    for (const lead of retained) {
      // Always fetch a real decision-maker (name / email / phone) for the lead.
      // Apollo mode enriches directly from Apollo; web mode researches the web.
      let contact = {};
      if (mode === 'apollo') {
        set('running', { step: `Finding a contact (${lead.company_name})`, written });
        try {
          contact = await apolloEnrichContact(lead.apollo_org_id, wl.market, lead.company_phone);
