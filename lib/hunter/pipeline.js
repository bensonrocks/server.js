...
async function runForOrg(org) {
  ...
  try {
    ...
    let candidates = await callAgent(client, fillPrompt(PROMPTS.scout, wl, cap),
    ...
    if (fresh.length) {
      ...
      verified = (Array.isArray(out) ? out : []).filter(v => v.verified);
    }
...
module.exports = { startRun, status, runForOrg, MODEL };
