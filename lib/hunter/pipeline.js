...
async function apolloPost(pathname, payload) {
  ...
  if (!r.ok) throw new Error(`Apollo ${pathname} -> ${r.status}${r.status === 401 ? ' (check APOLLO_API_KEY)' : ''}`);
  return r.json();
}
...
// Apollo-only discovery: find companies matching the org's wishlist (market +
// industries) that have a reachable decision-maker, and return them as pipeline
// candidates — no web search needed. Each candidate carries its Apollo contact.
async function apolloDiscover(wl, cap) {
  ...
  const payload = { per_page: Math.min(100, Math.max(25, cap * 4)), person_titles: APOLLO_TITLES };
  if (market) payload.person_locations = market.split(/[,\/&]|and/i).map(s => s.trim()).filter(Boolean);
  if (wl.industries) payload.q_keywords = wl.industries.split(/[,\n;]/).slice(0, 4).map(s => s.trim()).filter(Boolean).join(' ');
  let people;
  try {
    const data = await apolloPost('mixed_people/search', payload);
  ...
  for (const p of people) {
    ...
    const name = org.name || '';
    if (!name) continue;
    const domain = normDomain(org.website_url || org.primary_domain || '');
    const key = domain || normName(name);
    if (byCo.has(key)) continue;
    byCo.set(key, {
      company_name: name,
      website: org.website_url || (domain ? 'https://' + domain : ''),
      industry: org.industry || wl.industries || '',
      country_of_origin: market,
      sg_activity: `Matches your target profile in ${market || 'your market'}${org.industry ? ` (${org.industry})` : ''}; decision-maker reachable via Apollo.`,
      signal_code: 'SIG-ICP',
      signal_date: today,
      evidence_urls: [org.website_url || p.linkedin_url || ''].filter(Boolean),
      sources_confirmed: 1,
      _apolloContact: apolloContact(p),
    });
  }
...
async function runForOrg(org) {
  ...
  try {
    ...
    // Re-attach each candidate's Apollo contact + website (the analyst may drop them).
    if (mode === 'apollo') {
      ...
      for (const l of retained) {
        ...
        if (src) {
          if (!l._apolloContact) l._apolloContact = src._apolloContact;
    ...
    for (const lead of retained) {
      ...
      // Apollo mode already has the decision-maker from discovery — use it and
      // skip the web-search contact call. Web mode still researches contacts.
      let contact = lead._apolloContact || {};
      if (mode !== 'apollo') {
        try {
