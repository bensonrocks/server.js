...
// A short, factual "what they do" brief — prefer Apollo's own company
// description; otherwise synthesise from firmographics.
function companyBrief(cand) {
  if (cand.short_description) {
    const s = cand.short_description.replace(/\s+/g, ' ').trim();
    return s.length > 320 ? s.slice(0, 317).replace(/\s+\S*$/, '') + '…' : s;
  }
  const ind = cand.industry ? cand.industry.charAt(0).toUpperCase() + cand.industry.slice(1) : 'Company';
  const staff = parseInt(cand.estimated_num_employees, 10) || 0;
  const where = cand.country_of_origin ? ` based in ${cand.country_of_origin}` : '';
  return `${ind}${staff ? ` with about ${staff} staff` : ''}${where}.`;
}

// First couple of the operator's own services, for the "why approach" line.
function firstServices(services, n = 2) {
  const list = String(services || '').split(/[;,\n]/).map(s => s.trim()).filter(Boolean).slice(0, n);
  return list.length === 2 ? `${list[0]} and ${list[1]}` : list.join('');
}

// Why this lead is worth approaching now — grounded in the active signals and
// how the company maps to what the operator sells. Service-agnostic.
function whyApproach(cand, wl) {
  const sigs = (Array.isArray(cand.signals) ? cand.signals : []).map(s => SIGNAL_LABEL[s] || s);
  const svc = firstServices(wl.services, 2);
  const fit = svc ? ` a fit for your ${svc}` : ' a fit for what you offer';
  if (sigs.length) {
    return `They're ${sigs.join(' and ')} — a growth moment that usually creates new operational needs,${fit}. Worth a first touch before they commit to a provider.`;
  }
  return `Matches your target profile${cand.industry ? ` (${cand.industry})` : ''} —${fit}. Worth a first touch to gauge current needs.`;
}

// Enrich ONE decision-maker for a company: find a priority-title person (by
// Apollo org id, else company domain, else name), then reveal their name +
// email (+ phone when returned). Costs Apollo credits (people search + 1 per
// reveal), so it runs ONLY when the operator clicks "Enrich contact" on a lead.
// Degrades to a company-phone-only contact, then {}, on any failure.
async function apolloEnrichContact({ orgId, domain, companyName, market, companyPhone } = {}) {
  const fallback = companyPhone ? { phone: companyPhone, source: 'apollo' } : {};
  if (!process.env.APOLLO_API_KEY) return fallback;
  if (!orgId && !domain && !companyName) return fallback;
  const search = { person_titles: APOLLO_TITLES, per_page: 10, ...(market ? { person_locations: [market] } : {}) };
  if (orgId) search.organization_ids = [orgId];
  else if (domain) search.q_organization_domains_list = [domain];
  else search.q_keywords = companyName;

  let people = [];
  try { const s = await apolloPost('mixed_people/search', search); people = (s && s.people) || []; }
  catch { return fallback; }
  if (!people.length) return fallback;
  const rank = t => { const i = APOLLO_TITLES.findIndex(x => (t || '').toLowerCase().includes(x)); return i < 0 ? 99 : i; };
  people.sort((a, b) => rank(a.title) - rank(b.title));
  const pick = people[0];
  try {
    const m = await apolloPost('people/match', { id: pick.id, reveal_personal_emails: true });
    const p = (m && m.person) || {};
    const email = (p.email && p.email !== 'email_not_unlocked@domain.com') ? p.email
      : (p.personal_emails && p.personal_emails[0]) || (p.contact_emails && p.contact_emails[0] && p.contact_emails[0].email) || '';
    const phone = (p.phone_numbers && p.phone_numbers[0] && (p.phone_numbers[0].sanitized_number || p.phone_numbers[0].raw_number))
      || (p.contact && p.contact.sanitized_phone) || companyPhone || '';
    const name = p.name || [p.first_name, p.last_name].filter(Boolean).join(' ') || pick.name || '';
    return { name, title: p.title || pick.title || '', email, phone,
             profile_url: p.linkedin_url || pick.linkedin_url || '', source: 'apollo' };
  } catch {
    return { name: pick.name || '', title: pick.title || '', email: '',
             phone: companyPhone || '', profile_url: pick.linkedin_url || '', source: 'apollo' };
  }
}

// On-demand contact enrichment for a single stored lead (the "Enrich contact"
// button). Returns a contact object; {} / phone-only when Apollo has nobody.
async function enrichContact(org, lead) {
  const wl = (org && org.wishlist) || {};
  return apolloEnrichContact({
    orgId: lead.apollo_org_id || '',
    domain: normDomain(lead.website || ''),
...
const SIGNAL_LABEL = { hiring: 'actively hiring', growing: 'growing headcount', funded: 'recently funded' };
...
async function apolloDiscover(wl, cap) {
  ...
  const base = { per_page: Math.min(100, Math.max(25, cap * 3)), page: 1 };
  if (market) base.organization_locations = market.split(/[,\/&]|(?:\band\b)/i).map(s => s.trim()).filter(Boolean);
  const tags = (wl.industries || '').split(/[,\n;]/).map(s => s.trim()).filter(Boolean).slice(0, 8);
  if (tags.length) base.q_organization_keyword_tags = tags;

  // Apollo-native buying signals from the wishlist — layered onto the ICP query.
  const signals = [];
  const enriched = { ...base };
  if (wl.signal_hiring) {
    enriched.organization_num_jobs_range = { min: 1 };   // has active job postings
    const titles = String(wl.signal_hiring_titles || '').split(/[,\n;]/).map(s => s.trim()).filter(Boolean).slice(0, 10);
    if (titles.length) enriched.q_organization_job_titles = titles;
    signals.push('hiring');
  }
  if (wl.signal_growing) {
    enriched.organization_headcount_growth_past_n_months = 6;
    enriched.organization_headcount_growth_range = { min: 10 };
    signals.push('growing');
  }
  if (wl.signal_funded) {
    const d = new Date(); d.setMonth(d.getMonth() - 12);
    enriched.latest_funding_date_range = { min: d.toISOString().slice(0, 10) };
    signals.push('funded');
  }

  const run = async (payload) => {
    const data = await apolloPost('mixed_companies/search', payload);
    return (data && (data.organizations || data.accounts)) || [];
  };

  // Try the signal-filtered search first; if Apollo rejects a filter or it
  // returns nothing, fall back to plain ICP so a run is never accidentally empty.
  let orgs, applied = signals;
  try {
    orgs = await run(signals.length ? enriched : base);
  } catch (e) {
    if (!signals.length) return { candidates: [], error: String((e && e.message) || e) };
    try { orgs = await run(base); applied = []; }
    catch (e2) { return { candidates: [], error: String((e2 && e2.message) || e2) }; }
  }
  if (signals.length && !orgs.length) {
    try { orgs = await run(base); applied = []; } catch { /* keep empty */ }
  }

  const sigText = applied.map(s => SIGNAL_LABEL[s] || s).join(', ');
  const byCo = new Map();
  ...
  for (const org of orgs) {
    ...
    const industryPart = org.industry ? org.industry.charAt(0).toUpperCase() + org.industry.slice(1) + ' company' : 'Company';
    byCo.set(key, {
      company_name: name,
      website: org.website_url || (domain ? 'https://' + domain : ''),
      industry: org.industry || tags[0] || '',
      country_of_origin: hq || market,
      sg_activity: `${industryPart} in ${hq || market || 'your market'}${size ? `, ${size}` : ''} — ${sigText ? sigText + '.' : 'matches your target profile.'}`,
      signal_code: applied.length ? 'SIG-INTENT' : 'SIG-ICP',
      signal_date: today,
      signals: applied.slice(),
      short_description: String(org.short_description || org.seo_description || '').trim(),
      company_phone: org.phone || (org.primary_phone && org.primary_phone.number) || org.sanitized_phone || '',
  ...
  return { candidates: Array.from(byCo.values()).slice(0, cap * 2), signals: applied };
}
...
function apolloScore(cand, wl) {
  ...
  const sigs = Array.isArray(cand.signals) ? cand.signals : [];
  const breakdown = {
    market: 26 + (hqMatch ? 4 : 0),           // Apollo already filtered on location
    signal: Math.min(22, sigs.length * 10),   // hiring/growing/funded each add intent weight
    ...
    urgency: sigs.length ? 3 : 0,             // an active signal implies some timeliness
  };
  const score = Math.min(98, breakdown.market + breakdown.signal + breakdown.fit
    + breakdown.volume + breakdown.access + breakdown.urgency);
...
function buildLead(org, cand, contact, draft, score, breakdown, service, size, nextAction) {
  ...
  return {
    ...
    company_brief: cand.company_brief || '',
    why_approach: cand.why_approach || '',
    ...
    apollo_org_id: cand.apollo_org_id || '',
    company_phone: cand.company_phone || '',
...
async function runForOrg(org) {
  ...
  try {
      ...
      if (mode === 'apollo') {
        ...
        retained = verified.map(c => {
          ...
          return { ...c, score, score_breakdown: breakdown,
                   uld_service: sigs.length ? 'Buying signal' : 'ICP match',
                   opportunity_size: apolloSize(c),
                   company_brief: companyBrief(c),
                   why_approach: whyApproach(c, wl),
                   recommended_next_action: sigs.length
                     ? `Reach out while the signal is fresh (${sigs.map(s => SIGNAL_LABEL[s] || s).join(', ')}).`
                     : 'Review the fit and identify a decision-maker to approach.' };
    ...
    for (const lead of retained) {
      // Apollo mode: leave the contact empty at discovery time — the operator
      // enriches lead-by-lead with the "Enrich contact" button so credits are
      // only spent on the leads they choose. Web mode researches inline.
      let contact = {};
      if (mode !== 'apollo') {
        try {
      ...
      // A named/emailable contact means the draft has a real recipient.
      draft.requires_recipient = !(contact && (contact.email || contact.name));
...
module.exports = { startRun, status, runForOrg, startTest, testStatus, enrichContact, MODEL };
