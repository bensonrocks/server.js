...
module.exports = function createPickingWave(db) {
  ...
  const createWave = (options = {}) => {
    const { name, warehouseId, priority = 'normal', maxOrders = 50 } = options;

    const waveId = require('crypto').randomUUID();
    const now = new Date().toISOString();

    db.prepare(`
      INSERT INTO picking_waves (id, name, warehouse_id, status, priority, max_orders, created_at, updated_at)
      VALUES (?, ?, ?, 'created', ?, ?, ?, ?)
    `).run(waveId, name || `Wave-${Date.now()}`, warehouseId || null, priority, maxOrders, now, now);

    return { id: waveId, name, status: 'created', orders: 0 };
  };
