...
const { validateReturnItems } = require('./wms-validation');
...
module.exports = function createReturnsManager(db) {
  ...
  const createReturn = (params) => {
    ...
    const validation = validateReturnItems(items.map((item, i) => ({
      id: `item-${i}`,
      order_id: orderId,
      sku_id: item.sku_id,
      return_qty: item.qty,
    ...
    if (!validation.passed) {
      ...
      throw new Error(`Validation failed: ${criticalErrors.map(e => e.issue).join('; ')}`);
    }
