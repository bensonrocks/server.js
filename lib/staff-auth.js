...
function validateToken(token) {
  ...
  const row = mainDb.prepare('SELECT username, expires_at FROM staff_sessions WHERE token = ?').get(token);
  if (!row) return null;
...
function listStaff() {
  return mainDb.prepare('SELECT username, active, created_at FROM staff_users ORDER BY username').all();
}

module.exports = { checkPassword, generateToken, validateToken, revokeToken, changePassword, listStaff };
