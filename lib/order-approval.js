 ...
 */
module.exports = function createOrderApproval(db, clientConfig, virtualFulfillment = null, notifications = null) {
  /**
  ...
  const approvePendingOrder = (pendingOrderId, approvedBy) => {
    ...
    if (virtualFulfillment) {
      try {
        ...
        // Send notification to client about virtual items
        if (notifications && fulfillmentTracking.virtualItemsTracked > 0) {
          try {
            notifications.notifyOrderApproved(pending.client_id, orderId, fulfillmentTracking.items);
          } catch (err) {
            // Log but don't fail on notification
            console.error(`Failed to send order approved notification:`, err.message);
          }
        }
      } catch (err) {
