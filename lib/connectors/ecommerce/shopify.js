...
async function fetchWaybill(creds, externalId) {
  ...
  const res = await fetch(
    `https://${shop}/admin/api/${API_VER}/orders/${externalId}/fulfillments.json`,
    { headers: { 'X-Shopify-Access-Token': creds.accessToken } }
  );
  const j = await res.json();
  if (j.errors) throw new Error(typeof j.errors === 'string' ? j.errors : JSON.stringify(j.errors));
  const fulfillment = (j.fulfillments || [])[0];
  if (!fulfillment) throw new Error('No fulfillments found — order may not be fulfilled yet');
  return {
...
async function pushStatus(creds, externalId, status, opts = {}) {
  const shop = shopHost(creds.shopDomain || '');
  if (!shop) throw new Error('Shopify shop domain not configured');
  // Only push fulfillments when shipping; Shopify requires a fulfillment to be created
  if (status !== 'shipped') return;
  const payload = {
    fulfillment: {
      notify_customer: true,
      ...(opts.trackingNumber && { tracking_number: opts.trackingNumber }),
      ...(opts.carrier        && { tracking_company: opts.carrier }),
    },
  };
  const res = await fetch(
    `https://${shop}/admin/api/${API_VER}/orders/${externalId}/fulfillments.json`,
