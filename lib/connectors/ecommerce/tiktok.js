...
async function fetchWaybill(creds, externalId) {
  ...
  const ts     = Math.floor(Date.now() / 1000);
  const params = { app_key: appKey, timestamp: ts, version: '202309' };
  const body   = JSON.stringify({ order_id: externalId, document_type: 'SHIPPING_LABEL' });
  params.sign  = sign(appSecret, params, body);
  const res    = await fetch(
    `${API_BASE}/api/fulfillment/202309/packages/shipping_document?${new URLSearchParams(params)}`,
    { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-tts-access-token': accessToken }, body }
  );
  const j = await res.json();
  if (j.code !== 0) throw new Error(j.message || 'TikTok waybill fetch failed');
  const doc = j.data;
  if (!doc) throw new Error('No document data returned');
  if (doc.doc_url)     return { url: doc.doc_url };
  if (doc.doc_content) return { base64: doc.doc_content, contentType: 'application/pdf' };
  throw new Error('No waybill URL or content in response');
}
