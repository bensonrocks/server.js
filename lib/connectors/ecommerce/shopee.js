...
async function fetchWaybill(creds, externalId) {
  ...
  const res  = await fetch(
    `${BASE}${path}?partner_id=${partnerId}&timestamp=${ts}&sign=${sig}&access_token=${accessToken}&shop_id=${shopId}`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ order_list: [{ order_sn: externalId, shipping_document_type: 'SHIPPING_LABEL' }] }),
    }
  );
  const j = await res.json();
  if (j.error) throw new Error(j.message || 'Failed to get shipping document');
  const doc = (j.response?.result_list || [])[0];
  if (!doc) throw new Error('No shipping document returned');
  if (doc.fail_error) throw new Error(doc.fail_message || doc.fail_error);
  return { url: doc.shipping_document_url };
}
