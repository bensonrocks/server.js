...
function createWave({ orderIds, strategy = 'fifo', notes = '', skipUnavailable = false }) {
  ...
  db.transaction(() => {
    db.prepare(`
      INSERT INTO pick_waves (id, wave_number, strategy, status, notes)
      VALUES (?, ?, ?, 'open', ?)
    `).run(waveId, waveNumber, strategy, notes);
  ...
  return { waveId, waveNumber, strategy, orderCount: orders.length, taskCount: rawTasks.length, skippedOrders: skipped };
}
