...
// Lot/batch cell → string. Excel sometimes auto-formats numeric lot codes as
// dates (cellDates:true then yields a Date object); render those as YYYY-MM-DD
// instead of the unusable "Fri May 03 2030 00:00:00 GMT+0000 ..." form.
function lotStr(v) {
  if (v == null) return '';
...
function mapRow(rawRow, detected = {}) {
  ...
  // Betime "(017) SKU Lottables Issued" export splits quantity across
  // "Whole Qty" (full cartons) + "Loose Qty" (loose units), with "LHU Qty"
  // as the handling-unit total. Whole+Loose wins when either is positive;
  // LHU Qty covers rows where both are 0/empty (common — the qty then lives
  // only in the LHU column, e.g. 1 SET / 2 PACK).
  const _wholeLoose = (Number(n.whole_qty) || 0) + (Number(n.loose_qty) || 0);
  const rawQty =
    n.quantity ?? n.qty ?? n.pcs ?? n.pieces ??
    n.released_qty_sap ?? n.released_qty ??  // SAP/iWMS GI export: qty ordered (not yet-picked/shipped, which may be partial mid-process)
    n.d_expectedqty ??                       // Keyfields IssueDetail export: d-expectedqty
    (_wholeLoose > 0 ? _wholeLoose : undefined) ??
    (Number(n.lhu_qty) > 0 ? Number(n.lhu_qty) : undefined) ??
  ...
  return {
    // ── Order identifier ────────────────────────────────────────────────────
    // Unique-key priority (business rule): 1) GI No / GI Number / GINo,
    // 2) Waybill No, 3) Order reference. One of these MUST resolve on every
    // upload — the upload pipeline rejects the file otherwise.
    // Also includes Keyfields WMS output column (d-exref2) and Betime internal
    // refs (Issue No, PickTicket) so Keyfields-generated picklists import
    // without the "UNKNOWN order number" error.
    order_number: String(
      // 1) GI (Goods Issue) number — primary WMS order key. Each GI row IS one
      // goods-issue transaction (same as the GI-#### barcode the PDF
      // picking-list path already turns into order_number).
      n.gino             ??   // iWMS "Good Issue Analysis" export: "GINo"
      n.gi_no            ??
      n.gi_number        ??
      n.iwms_gino        ??   // SAP/iWMS GI export: "iWMS GINo"
      n.gi               ??
      n.goods_issue_no   ??
      n.goods_issue_number ??
      n.goods_issue      ??
      // 2) Waybill / AWB / tracking number
      n.waybill_number   ??
      n.waybill_no       ??
      n.waybill          ??
      n.awb              ??
      ...
      n.airway_bill      ??
      n.tracking_number  ??
      n.tracking_no      ??   // SAP/iWMS GI export: "Tracking no"
      // 3) Order reference
      n.ref              ??
      n.order_number     ??
      n.order_no         ??
      n.order            ??   // bare "Order" column header
      n.so_number        ??
      n.sales_order      ??
      n.job_no           ??
      n.reference        ??
    ...
    sku: String(
      n.product_code     ??
      n.sku              ??
      n.sku_code         ??   // Betime Lottables export: "SKU Code"
      n.item_code        ??
      n.barcode          ??
      n.product_sku      ??
      n.item_sku         ??
    ...
    description: cleanDescription(
      n.description         ??
      n.item_description    ??
      n.sku_descr           ??   // Betime Lottables export: "SKU Descr"
      n.sku_description     ??
    ...
    batch_number: lotStr(
      ...
      n.lot_number       ??
      n.lot_no           ??
      n.batchno_lotno    ??   // Betime Lottables export: "BatchNo/LotNo"
    ...
    expiry_date: dateVal(
      n.expiry_date      ??
      n.expirydate_lot1  ??   // Betime Lottables export: "ExpiryDate/Lot1"
      n.expiry           ??
      n.best_before      ??
      n.exp_date         ??
      n.bb_date          ??
    ...
    // Unit of measure for the line. For the Betime Lottables export the
    // active UOM follows whichever qty column supplied the quantity:
    // Loose Qty → Loose UOM, Whole Qty → Whole UOM, LHU Qty → LHUUOM.
    uom: String(
      n.uom              ??
      n.unit             ??
      n.unit_of_measure  ??
      (Number(n.loose_qty) > 0 ? n.loose_uom : undefined) ??
      (Number(n.whole_qty) > 0 ? n.whole_uom : undefined) ??
      (Number(n.lhu_qty)   > 0 ? n.lhuuom    : undefined) ??
      ''
    ).trim(),
