...
function detectColumnMap(rawRows) {
  ...
  function scoreSku(col) {
    ...
    if (/^(location|loc|bin|bin_code|bin_loc|bin_location|storage_loc|storage_location|warehouse_loc|rack|shelf|zone|aisle|slot|loc_code|location_code)$/.test(col)) return -99;
    let score = 0;
...
function mapRow(rawRow, detected = {}) {
  ...
  return {
    ...
    // Warehouse bin/location this SKU is picked FROM (e.g. "99-001-011",
    // AC-007-003-B). Wave picking groups its consolidated pick list by
    // location + SKU so a packer walks bin-to-bin instead of picking a SKU
    // blind without knowing where it physically sits.
    location: String(
      n.location         ??
      n.loc              ??
      n.bin               ??
