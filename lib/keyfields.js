...
// One shape test for "this consignee is really a waybill", shared with the
// picking-list parsers (see lib/ocr-parse.js) so the three cannot disagree.
const { looksLikeTrackingNumber } = require('./ocr-parse');
...
// Warehouse bin/location column names (post-normalizeKey, e.g. "Bin No." ->
// "bin_no"). Used in TWO places that must stay in sync: scoreSku() excludes
// these from ever being read as the SKU column, and mapRow()'s location
// alias chain reads location FROM these same column names — location is now
// the primary input for wave picking (lib/wave-pick.js), so real-world
// header-name coverage here matters.
const LOCATION_COLUMN_NAMES = [
  'location', 'loc', 'bin', 'bin_code', 'bin_loc', 'bin_location', 'bin_no', 'bin_number', 'bin_id',
  'storage_loc', 'storage_location', 'storage_bin', 'warehouse_loc', 'warehouse_location',
  'wh_location', 'wh_loc', 'pick_location', 'pick_loc', 'putaway_location',
  'rack', 'rack_no', 'rack_number', 'shelf', 'shelf_no', 'shelf_number',
  'zone', 'aisle', 'slot', 'slot_no', 'slot_number',
  'loc_code', 'location_code', 'location_no', 'location_number', 'location_id',
];
...
// A TEXT field must never render a raw JavaScript Date. XLSX parses a
// date-formatted cell into a Date object, and a bare String() on it puts
// "Sat Oct 28 2028 00:00:00 GMT+0000 (Coordinated Universal Time)" on the
// packer's screen where a lot code belongs — seen on a real BETIME line whose
// BatchNo/LotNo cell was date-formatted. Any date reaching a text field is
// written as a plain calendar day instead.
function textVal(v) {
...
function detectColumnMap(rawRows) {
  ...
  // ── Keyfields / IssueDetail schema fast-path ───────────────────────────────
  // Files in the Keyfields WMS "d-" column schema (d-SKUCODE, d-exref2,
  // d-expectedqty, …) are a KNOWN layout — never run heuristic scoring on
  // them. Heuristics once picked d-exline (the line NUMBER) as the qty column
  // because "expectedqty" missed the old ^qty$ keyword regex and the two
  // tied on numeric stats. `explicit: true` also tells the upload pipeline
  // that SKUs from this map are schema-certain (no location-code suspicion).
  {
    const firstKeys = new Set();
    for (const row of rawRows.slice(0, 5)) {
      for (const k of Object.keys(row)) firstKeys.add(normalizeKey(k));
    }
    if (firstKeys.has('d_skucode')) {
      const result = { explicit: true, schema: 'keyfields', sku_key: 'd_skucode' };
      if (firstKeys.has('d_exref2'))      result.order_key = 'd_exref2';
      if (firstKeys.has('d_expectedqty')) result.qty_key   = 'd_expectedqty';
      else if (firstKeys.has('qty'))      result.qty_key   = 'qty';
      if (firstKeys.has('d_skudesc'))     result.desc_key  = 'd_skudesc';
      return result;
    }
  }
  ...
  for (const col of columns) {
    ...
    // Date cells (XLSX cellDates:true yields Date objects; strings may be
    // date-formatted too) — a date column must never become the description
    const dateCount = vals.filter(v =>
      v instanceof Date ||
      /GMT[+-]\d{4}/.test(String(v)) ||
      /^\d{4}-\d{2}-\d{2}(T|\s|$)/.test(String(v).trim()) ||
      /^\d{1,2}[\/-]\w{3,}[\/-]\d{2,4}$/.test(String(v).trim())
    ).length;
    stats[col] = {
      fill:        vals.length / n,
      unique,
      ...
      dateRatio:   dateCount / vals.length,
      avgLen:      strs.reduce((sum, s) => sum + s.length, 0) / strs.length,
      // Every sampled row holds the literal same value AND it's "0" — e.g. a SAP
      // "Released Qty" column reading 0 on every line of a picking list because
      // nothing has been released yet. Used to stop scoreQty() rewarding this
      // as if it were a genuine small-variety qty column (1, 2, 3…).
      allZero:     unique === 1 && strs[0] === '0',
    };
  }
  ...
  function scoreSku(col) {
    const s = stats[col];
    if (!s || s.fill < 0.4) return -99;
    // Warehouse location/bin address columns are never product SKUs
    if (LOCATION_COLUMN_NAMES.includes(col)) return -99;
    let score = 0;
  ...
  function scoreQty(col) {
    ...
    if (!s) return -99;
    let score = 0;
    // Substring match on qty/quantity so prefixed variants (expectedqty,
    // d_expectedqty, ship_qty…) get the keyword bonus too
    if (/qty|quantity|^pcs$|pieces|^count$|^units?$|^amount$/.test(col)) score += 20;
    // Line/sequence-number columns are numeric, short, low-cardinality — they
    // look EXACTLY like a qty column to the stats below. Penalise them by
    // name so d_exline / SNo / Seq never beat a real qty column on a tie.
    if (/line|seq|serial|s_?no$|^no$|^row/.test(col)) score -= 15;
    if (s.numRatio > 0.9)     score += 15;
    if (s.avgLen <= 3)        score += 8;
    // A column that is literally 0 on every single row is almost never a real
    // per-line quantity (no picking list genuinely asks to pick 0 of
    // everything) — it's a placeholder/not-yet-populated field (e.g. SAP
    // "Released Qty" before goods-issue). Never let it win on the
    // low-cardinality bonus meant for realistic small-variety qty columns.
    if (s.allZero)           score -= 25;
    else if (s.cardinality <= 0.2) score += 3; // small set of repeated values (1, 2, 3…)
    return score;
  }

  // Header words that mean "this column holds a code for something", so the
  // column can never be a product description no matter how its values score.
  const DESC_NEVER_WORDS = new Set([
    'waybill', 'tracking', 'awb', 'airway', 'consignment',
    'barcode', 'ean', 'upc', 'invoice', 'shipment',
  ]);
  function scoreDesc(col) {
    const s = stats[col];
    if (!s || s.fill < 0.3) return -99;
    if (s.dateRatio > 0.5)  return -99;  // date columns are never descriptions
    // IDENTIFIER COLUMNS ARE NEVER DESCRIPTIONS. A real client file with
    // columns "SO Ref | Waybill Ref | SKU | Qty" had "Waybill Ref" scored as
    // the description, so every pick line would have read "LZSGD1015082878"
    // where the product name belongs. Matched on whole WORDS of the normalised
    // header, not as a substring — "Order Notes" (order_notes) is a perfectly
    // good description and must not be caught by an "order_no" substring.
    if (col.split('_').some(t => DESC_NEVER_WORDS.has(t))) return -99;
    let score = 0;
    if (/desc|description|product_name|item_name|goods_name|commodity|title|label|specification|spec|detail|remark/.test(col)) score += 18;
    if (/^name$|^product$|^goods$|^item$|^model$/.test(col)) score += 12;
    // Descriptions are text, not numbers
    if (s.numRatio < 0.1)  score += 8;
    else if (s.numRatio > 0.5) return -99;
    // Descriptions are longer than codes
    if (s.avgLen > 8)  score += 6;
    else if (s.avgLen < 3) return -99;
    // Descriptions are reasonably unique
    if (s.cardinality >= 0.5) score += 4;
    return score;
  }
  ...
  for (const col of columns) {
    ...
    if (ds > bestDescScore)  { bestDescScore  = ds; bestDesc  = col; }
  }

  // Don't let desc_key steal a column already claimed by a higher-priority role
  const taken = new Set([bestOrder, bestSku, bestQty]);
  ...
  if (bestDescScore  >= 5 && !taken.has(bestDesc))      result.desc_key  = bestDesc;
...
// Descriptions must be product text, never dates — XLSX parsing with
// cellDates:true turns date cells into Date objects, which stringify to
// "Fri Jun 12 2026 00:00:00 GMT+0000 (Coordinated Universal Time)".
function cleanDescription(v) {
  if (v === null || v === undefined) return '';
  if (v instanceof Date) return '';
  const s = String(v).trim();
  if (/GMT[+-]\d{4}/.test(s)) return '';
...
function mapRow(rawRow, detected = {}) {
  ...
  // "Released Qty (SAP)" is 0 on an iWMS Good Issue Analysis export for EVERY
  // row until goods-issue actually happens — on a picking list nothing has
  // been released yet, so this column is a false zero, not a genuine
  // zero-quantity line (a real 0-qty pick line makes no operational sense).
  // Checking it first (as this used to) meant `??` accepted the 0 immediately
  // and never looked at the real expected/ordered-qty column, poisoning
  // EVERY row with qty=0 and aborting the whole upload on INVALID QUANTITY.
  // Named/expected-qty columns and the AI-detected qty column are tried
  // FIRST; released_qty_sap/released_qty are last-resort, and even then a
  // literal 0 from them is skipped in favour of the final `1` default.
  const qtyCandidates = [
    n.quantity, n.qty, n.pcs, n.pieces,
    n.d_expectedqty,                                    // Keyfields IssueDetail export: d-expectedqty
    (detected.qty_key ? n[detected.qty_key] : undefined),
    n.released_qty_sap, n.released_qty,                 // SAP/iWMS GI export: last resort — see note above
  ];
  let rawQty;
  ...
  return {
    ...
    order_number: textVal(
      n.ref              ??
      n.order_number     ??
      n.order_no         ??
      n.order_ref        ??
      n.so_ref           ??   // "SO Ref" — a real client's outbound file
      n.so_no            ??
      n.so_number        ??
      n.sales_order      ??
      n.sales_order_no   ??
      ...
      n.gino             ??   // iWMS "Good Issue Analysis" export: "GINo" column — each
      n.gi_no            ??   // row IS one goods-issue transaction, so GI number is the
      n.gi_number        ??   // order identifier here (same as the GI-#### barcode the
                               // PDF picking-list path already turns into order_number)
      ...
      n.shipment_number  ??
      n.dispatch_no      ??
      n.pick_list_no_sap_pick_list_customerreference ??  // SAP/iWMS GI export: "Pick List No (SAP Pick List/CustomerReference)"
      n.doc_no_sap_so_no ??  // SAP/iWMS GI export: "Doc. No (SAP SO No)"
    ...
    customer_name: textVal(
      n.customer_name  ??
      // Keyfields prints the TracX waybill in the Consignee column on the
      // Betime list — a tracking number is the waybill, never the customer.
      (looksLikeTrackingNumber(n.consignee) ? undefined : n.consignee) ??
      n.recipient      ??
      n.ship_to_name   ??
    ...
    // ── Logistics ───────────────────────────────────────────────────────────
    waybill_number: textVal(
      n.tracking_number  ??
      n.tracking_no      ??   // "Tracking No" abbreviated header (e.g. Lazada ECOM)
      n.waybill_number   ??
      n.waybill_no       ??
      n.waybill_ref      ??   // "Waybill Ref" — a real client's outbound file
      n.waybill          ??
      n.tracking         ??
      n.tracking_ref     ??
      // A tracking-shaped Consignee IS the waybill (the Betime picking-list
      // shape). Placed here so any named tracking column above still wins.
      (looksLikeTrackingNumber(n.consignee) ? String(n.consignee).replace(/[\s\-]/g, '').toUpperCase() : undefined) ??
      n.awb              ??
      n.awb_no           ??
      n.awb_number       ??
      n.airway_bill      ??
    ...
    issue_no: textVal(
      ...
      n.issue_ref         ??
      n.issue             ??
      n.iwms_gino         ??   // SAP/iWMS GI export: "iWMS GINo" (goods-issue no.)
      // THE GI MUST HAVE A SECOND PLACE TO LAND, OR IT IS SILENTLY LOST.
      // These three aliases are in the order_number chain above, but ELEVENTH —
      // behind `ref`/`reference`/`order_no`/`so_ref`/… So on a Betime GI
      // Analysis export whose GI column is spelled "GI No" (not "iWMS GINo")
      // AND which also carries a populated Reference column, `reference` won
      // order_number and the GI landed NOWHERE: not order_number, not issue_no.
      // Reported from the floor as "cannot scan picklist" — correctly, since
      // the GI-###### barcode printed on every picking list then matched no
      // stored field at all (the scan-to-find bar checks order_number /
      // issue_no / pick_ticket / waybill_number / po_number).
      // Deliberately added HERE rather than reordering the order_number chain:
      // order_number is the primary key for duplicates, scanning, labels and
      // reports, so moving it is a much larger change than closing this hole.
      n.gino              ??
      n.gi_no             ??
      n.gi_number         ??
      ''
    ...
    pick_ticket: textVal(
      ...
      n.pt_no              ??
      n.pick_no            ??
      n.iwms_pickticket    ??   // SAP/iWMS GI export: "iWMS PickTicket"
      ''
    ),

    // PO / shipment number (SHPM…) — scannable off picking lists, so it must
    // survive spreadsheet uploads too for the waybill-lookup to find it
    po_number: textVal(
      n.po_number          ??
      n.po_no              ??
      n.po                 ??
      ...
      n.shipment_number    ??
      n.shpm_no            ??
      n.iwms_dono          ??   // SAP/iWMS GI export: "iWMS DONo" (delivery order no.)
      ''
    ),
    ...
    date: dateVal(
      ...
      n.delivery_date    ??
      n.dispatch_date    ??
      n.expected_date    ??   // SAP/iWMS GI export: "Expected Date"
    ...
    sku: textVal(
      ...
      n.item_sku         ??
      n.article_no       ??
      n.sku_code         ??   // GI Analysis export: "SKU Code"
      n.skucode          ??
    ...
    // ── Warehouse bin/location for this line, if the file has one ────────────
    // Never used to identify the SKU — scoreSku() excludes the exact same
    // LOCATION_COLUMN_NAMES from ever being read as the SKU column. This is
    // the primary input for location-sorted wave picking (lib/wave-pick.js).
    location: textVal(
      LOCATION_COLUMN_NAMES.map(c => n[c]).find(v => v != null) ?? ''
    ).trim(),

    // Where the SKU value came from — 'schema' when a KNOWN named column (the
    // alias chain above, or an explicit schema map like Keyfields d-SKUCODE)
    // supplied it, 'detected' when only the AI column-scoring fallback did.
    // The upload pipeline trusts schema SKUs outright but treats detected
    // SKUs shaped like warehouse location codes as needing user confirmation.
    _skuSource: (
      (n.product_code ?? n.sku ?? n.item_code ?? n.barcode ?? n.product_sku ??
       n.item_sku ?? n.article_no ?? n.d_skucode ?? n.item_no ?? n.item ??
       n.material ?? n.material_no ?? n.mat_no ?? n.part_no ?? n.part_number ??
       n.product_id ?? n.code) != null || detected.explicit
        ? 'schema'
        : (detected.sku_key && n[detected.sku_key] != null ? 'detected' : 'none')
    ),
    ...
    description: cleanDescription(
      n.description         ??
      n.sku_descr           ??   // GI Analysis export: "SKU Descr"
      n.sku_description     ??
      n.item_description    ??
      ...
      n.goods               ??
      n.desc                ??
      (detected.desc_key ? n[detected.desc_key] : undefined) ??
      ''
    ),

    // ── WMS extra fields ────────────────────────────────────────────────────
    batch_number: textVal(
      n.batch_number     ??
      n.lot_number       ??
      n.lot_no           ??
      n.batchno_lotno    ??   // GI Analysis export: "BatchNo/LotNo"
    ...
    expiry_date: dateVal(
      n.expirydate_lot1  ??   // GI Analysis export: "ExpiryDate/Lot1"
      n.expiry_date      ??
      n.expiry           ??
      n.best_before      ??
      n.exp_date         ??
