...
// Infer the carrier from the tracking-number prefix when the file has no
// carrier column (e.g. Betime GI Analysis exports carry only "Tracking no").
// Only prefixes confirmed from real shipment data — extend as new ones appear.
const TRACKING_CARRIER_PREFIXES = [
  [/^SPTTND/i,        'SingPost'],
  [/^SPXSG/i,         'SPX Express'],
  [/^NLSGD|^NVSGD/i,  'Ninja Van'],
  [/^LZSGD/i,         'Lazada Logistics'],
  [/^SHPM/i,          'Shopee'],
  // TXSGD… prefix seen in real data but courier unconfirmed — add when known.
];
function inferCarrierFromTracking(tracking) {
  const t = String(tracking || '').trim();
  if (!t) return '';
  for (const [re, name] of TRACKING_CARRIER_PREFIXES) {
...
function detectColumnMap(rawRows) {
  ...
  function scoreOrder(col) {
    ...
    let nameScore = 0;
    if (/order|ref|invoice|shipment|consign|^gi$|^gi_|goods_issue|waybill|^awb|^po$|^po_|_po$/.test(col)) nameScore += 15;
...
function mapRow(rawRow, detected = {}) {
  ...
  const rawQty =
    ...
    n.released_qty_sap ?? n.released_qty ??   // Betime GI Analysis: "Released Qty (SAP)"
    n.allocated_qty ??
  ...
  // Waybill / tracking number, hoisted so the carrier fallback below can
  // infer the courier from its prefix.
  const _waybillVal = String(
    n.tracking_number  ??
    n.tracking_no      ??   // Betime GI Analysis export: "Tracking no"
    n.waybill_number   ??
    n.waybill_no       ??
    n.waybill          ??
    n.awb_no           ??
  ...
  return {
    // ── Order identifier ────────────────────────────────────────────────────
    // Unique-key priority (business rule): 1) GI No / GI Number,
    // 2) Waybill No, 3) Order reference. One of these MUST resolve on every
    // upload — /api/upload rejects the file otherwise.
    ...
    order_number: String(
      // 1) GI (Goods Issue) number — primary WMS order key
      n.gi_no            ??
      n.gi_number        ??
      n.gino             ??   // "GINo" spelling
      n.iwms_gino        ??   // Betime GI Analysis export: "iWMS GINo"
      n.gi               ??
      n.goods_issue_no   ??
    ...
    pick_ticket: String(
      n.pick_ticket        ??
      n.pickticket         ??
      n.iwms_pickticket    ??   // Betime GI Analysis export: "iWMS PickTicket"
      n.pick_ticket_no     ??
      n.pickticket_no      ??
    ...
    // Marketplace / customer order reference (e.g. Betime "Cust Ref" —
    // Shopee/Lazada order numbers like 260725K6HB45JW or 170475932924488).
    // Carrier labels print THIS number (plus the tracking no), never the GI
    // number, so it must be indexed for waybill-PDF page matching.
    customer_ref: lotStr(
      n.cust_ref            ??
      n.customer_reference  ??
      n.customer_ref        ??
      n.ship_reference      ??   // Betime Lottables export: "Ship Reference"
      n.pick_list_no_sap_pick_list_customerreference ??  // (101) GI with SAP Info export
      ''
    ),

    // Explicit carrier column wins; otherwise inferred from the tracking-
    // number prefix (SPTTND → SingPost, SPXSG → SPX Express, …) so exports
    // that carry only a tracking number still show a carrier.
    carrier: String(
      n.driver           ??
      n.carrier          ??
      n.courier          ??
      ...
      ''
    ) || inferCarrierFromTracking(_waybillVal),
    ...
    date: dateVal(
      ...
      n.delivery_date    ??
      n.dispatch_date    ??
      n.expected_date    ??   // Betime GI Analysis export: "Expected Date"
    ...
    description: String(
      n.description      ??
      n.item_description ??   // Betime GI Analysis export: "Item Description"
      ...
      n.desc             ??
      n.item_desc        ??
      n.d_rem3           ??   // Keyfields output: d-rem3 (product description)
      ''
    ),
