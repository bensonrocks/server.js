...
async function getWaybill(creds, externalId, orderItemIds) {
  const { appKey, appSecret, accessToken, region = 'MY' } = creds;
  const base = API_BASE[region] || API_BASE.MY;
  const ts   = Date.now().toString();
  const params = {
    ...
    order_item_ids: JSON.stringify(Array.isArray(orderItemIds) ? orderItemIds : [orderItemIds]),
  };
  params.sign = sign(appSecret, '/orders/document/get', params);
  const res = await fetch(`${base}/rest/orders/document/get?${new URLSearchParams(params)}`);
  const j   = await res.json();
  if (j.code !== '0') throw new Error(j.message || 'Lazada waybill fetch failed');
  return { url: j.data?.document?.file_path || null };
}
...
async function pushStatus(creds, externalId, status, trackingNo) {
  ...
  if (status === 'processing') {
    const params = { app_key: appKey, timestamp: ts, sign_method: 'sha256', access_token: accessToken, order_id: externalId };
    params.sign  = sign(appSecret, '/orders/pack', params);
    const res = await fetch(`${base}/rest/orders/pack?${new URLSearchParams(params)}`, { method: 'POST' });
    const j   = await res.json();
    if (j.code !== '0') throw new Error(j.message || 'Lazada pack failed');
    return { ok: true };
  }

  if (status === 'shipped') {
    if (!trackingNo) throw new Error('Tracking number required for Lazada shipped status');
    const params = { app_key: appKey, timestamp: ts, sign_method: 'sha256', access_token: accessToken, order_id: externalId, tracking_number: trackingNo };
    params.sign  = sign(appSecret, '/orders/rts', params);
    const res = await fetch(`${base}/rest/orders/rts?${new URLSearchParams(params)}`, { method: 'POST' });
    const j   = await res.json();
    if (j.code !== '0') throw new Error(j.message || 'Lazada RTS failed');
    return { ok: true };
  }
