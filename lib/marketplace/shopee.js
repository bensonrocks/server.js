...
async function getWaybill(creds, orderSn) {
  const { partnerId, partnerKey, accessToken, shopId } = creds;
  const ts   = Math.floor(Date.now() / 1000);
  ...
  const res  = await fetch(`${BASE}${path}?partner_id=${partnerId}&timestamp=${ts}&sign=${sig}&access_token=${accessToken}&shop_id=${shopId}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ order_list: [{ order_sn: orderSn, package_number: '' }], shipping_document_type: 'THERMAL_AIR_WAYBILL' }),
  });
  if (!res.ok) throw new Error('Shopee waybill fetch failed: ' + res.status);
  const contentType = res.headers.get('content-type') || '';
  if (contentType.includes('application/pdf') || contentType.includes('application/octet-stream')) {
    const buf = await res.arrayBuffer();
    return { pdf: Buffer.from(buf) };
  }
  const j = await res.json();
  if (j.error) throw new Error(j.message || 'Shopee waybill error');
  return { url: j.response?.result || null };
}
