  ...
  const createInternalOrder = (po, destinationStore, lineItems) => {
    ...
    try {
      ...
      lineItems.forEach((item, idx) => {
        ...
        // Lookup sku_id from sku_code (required for allocator)
        const sku = db.prepare('SELECT id FROM skus WHERE code = ?').get(item.sku_code);
        const skuId = sku ? sku.id : null;

        db.prepare(`
          INSERT INTO order_lines (
            id, order_id, sku_id, sku_code, sku_name, ordered_qty, line_number,
