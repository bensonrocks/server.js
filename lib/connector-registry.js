...
registry['zetpy'] = {
  meta: {
    ...
    authType:         'password',
    requiredForOAuth: [],
    regions:          ['MY', 'SG', 'ID', 'TH', 'PH', 'VN'],
    defaultStoreName: 'Zetpy Store',
    requiresLicense:  true,
  },
  fetchOrders: (creds, opts) => gateway.fetchOrders('zetpy', creds, opts),
  mapOrder: (order) => {
  ...
  pushStatus:  (creds, externalId, _status, trackingNo) =>
    gateway.pushShipment('zetpy', creds, {
      externalOrderId: String(externalId),
      trackingNumber:  trackingNo || '',
      notifyCustomer:  true,
    }),
  fetchWaybill: (creds, externalId) => gateway.fetchWaybill('zetpy', creds, externalId),
  buildAuthUrl:  null,
  exchangeCode:  null,
};
