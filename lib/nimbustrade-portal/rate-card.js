...
function computeLiveIndicative(clientId) {
  ...
  const markets = rows.map((r) => {
    ...
    return {
      ...
      total,
      // Full breakdown so the client UI can show exactly how the fee was
      // built, not just the final number.
      breakdown: {
        firstItemRate: tier.firstItemRate,
        additionalItemRate: tier.additionalItemRate || 0,
        additionalItems,
        fulfilmentFee,
        despatched: r.despatched,
        deliveryRate: DOMESTIC_DELIVERY_STANDARD_RATE,
        deliveryFee,
      },
    };
  });
