...
function listInboundForClient(clientId) {
  return db.prepare(`
    SELECT s.id, s.reference, s.carrier, s.waybill_number, s.contents, s.expected_qty, s.received_qty,
...
function createInbound(clientId, { locationId, reference, carrier, waybillNumber, contents, expectedQty, expectedDate }) {
  ...
  const id = uid('inb');
  db.prepare(`
    INSERT INTO nt_inbound_shipments (id, client_id, location_id, reference, carrier, waybill_number, contents, expected_qty, expected_date)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, clientId, locationId, reference, carrier || '', waybillNumber || '', contents || '', expectedQty || 0, expectedDate);
  return db.prepare('SELECT * FROM nt_inbound_shipments WHERE id = ?').get(id);
}

function updateInboundByStaff(id, { status, receivedQty, arrivedDate }) {
  const sets = [];
  const params = [];
  if (status) {
