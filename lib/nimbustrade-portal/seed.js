...
function seedBWLDemo() {
  ...
  const insertOrder = db.prepare(`
    INSERT INTO nt_orders (id, client_id, order_ref, country, country_name, customer_name, sku, product_name, qty, status, issue_note, order_date, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
    ...
    for (const { loc, count } of weights) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        insertOrder.run(
          ...
          customerName, item.sku, item.name, qty, status, issueNote, orderDate, createdAt
        );
      }
    }
  });
