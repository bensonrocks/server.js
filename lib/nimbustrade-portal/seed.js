...
function seedLocationOrders(loc, vendorId, rng, startSeq) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (status !== 'dropped') {
      insertEvent.run(uid('evt'), id, 'processing', '', `${addDays(orderDate, 1)} 10:00:00`);
    }
    if (status === 'completed') {
      insertEvent.run(uid('evt'), id, 'completed', '', `${addDays(orderDate, 2 + Math.floor(rng() * 3))} 16:00:00`);
    } else if (status === 'issue') {
      insertEvent.run(uid('evt'), id, 'issue', issueNote, `${addDays(orderDate, 2)} 13:00:00`);
    }
  }
  return orderSeq;
}
...
function seedBWLDemo() {
  ...
  const backfill = db.transaction(() => {
    ...
    // Locations that existed before inbound shipments did — seedLocation()
  ...
  if (!missing.length && !backfilledEvents && !backfilledInbound) return { alreadySeeded: true };
  return {
    alreadySeeded: true,
