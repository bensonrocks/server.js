...
// Two specific, deterministic showcase shipments — real airfreight ex-Singapore
// still in transit, one to each of the UK and US DCs — on top of the random
// per-location seeding above, so the map always has at least one clearly
// "moving right now" trail on each of those two routes. Idempotent by
// reference, so safe to call on every boot (including pre-existing deployments
// via the backfill path below).
function seedShowcaseAirfreightShipments() {
  const showcase = [
    {
      reference: 'INB-GB-006', locationId: 'loc-gb', carrier: 'DHL', waybillNumber: '2578134490',
      contents: 'Radiance Serum 30ml x120, Renewal Night Cream 50ml x80',
      expectedQty: 200, expectedDate: '2026-08-06',
    },
    {
      reference: 'INB-US-006', locationId: 'loc-us', carrier: 'FedEx', waybillNumber: '884215307733',
      contents: 'Collagen Essence 30ml x150, Vitamin C Cleanser 100ml x90',
      expectedQty: 240, expectedDate: '2026-08-08',
    },
  ];
  let seeded = 0;
  for (const s of showcase) {
    const exists = db.prepare('SELECT id FROM nt_inbound_shipments WHERE reference = ?').get(s.reference);
    const loc = db.prepare('SELECT id FROM nt_locations WHERE id = ?').get(s.locationId);
    if (exists || !loc) continue;
    insertInbound.run(
      uid('inb'), CLIENT_ID, s.locationId, s.reference, s.carrier, s.waybillNumber,
      s.contents, s.expectedQty, 0, 'in_transit', s.expectedDate, ''
    );
    seeded += 1;
  }
  return seeded;
...
function seedBWLDemo() {
  ...
  if (!exists) {
    const seedAll = db.transaction(() => {
      ...
      seedShowcaseAirfreightShipments();
    });
    seedAll();
  ...
  const backfill = db.transaction(() => {
    ...
    backfilledShowcase = seedShowcaseAirfreightShipments();
  });
  backfill();

  if (!missing.length && !backfilledEvents && !backfilledInbound && !backfilledMonths.length && !backfilledShowcase) {
