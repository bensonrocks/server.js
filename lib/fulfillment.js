...
module.exports = function createFulfillment({ store, creds, inventory, db }) {
  ...
  async function requestCancellation(orderId, reason = '', requestedBy = 'system') {
    ...
    const existing = db.prepare('SELECT id FROM cancellation_requests WHERE order_id = ? AND status = ?').get(orderId, 'pending');
  ...
  async function approveCancellation(requestId, approvedBy = 'system') {
    const request = db.prepare('SELECT * FROM cancellation_requests WHERE id = ?').get(requestId);
