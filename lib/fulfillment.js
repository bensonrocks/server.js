...
module.exports = function createFulfillment({ store, creds, inventory, db }) {
  ...
  // Request cancellation — creates pending approval record, does NOT cancel immediately
  async function requestCancellation(orderId, reason = '', requestedBy = 'system') {
    const order = store.getOrder(orderId);
    if (!order) { const e = new Error('Order not found'); e.status = 404; throw e; }
    if (order.status === 'cancelled') throw Object.assign(new Error('Order is already cancelled'), { status: 400 });
    if (order.status === 'shipped') throw Object.assign(new Error('Cannot cancel shipped orders'), { status: 400 });

    // Check if already pending cancellation
    const existing = store.db.prepare('SELECT id FROM cancellation_requests WHERE order_id = ? AND status = ?').get(orderId, 'pending');
    if (existing) throw Object.assign(new Error('Cancellation already pending approval'), { status: 400 });

    const requestId = require('crypto').randomUUID();
    const now = new Date().toISOString();

    db.prepare(`
      INSERT INTO cancellation_requests (id, order_id, reason, status, requested_by, requested_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(requestId, orderId, reason, 'pending', requestedBy, now);
  ...
  // Approve cancellation — actually cancels order and releases inventory
  async function approveCancellation(requestId, approvedBy = 'system') {
    const request = store.db.prepare('SELECT * FROM cancellation_requests WHERE id = ?').get(requestId);
    if (!request) { const e = new Error('Cancellation request not found'); e.status = 404; throw e; }
    if (request.status !== 'pending') throw Object.assign(new Error('Cancellation not pending'), { status: 400 });

    const order = store.getOrder(request.order_id);
    if (!order) { const e = new Error('Order not found'); e.status = 404; throw e; }

    const previousStatus = order.status;
    const now = new Date().toISOString();

    // Update cancellation request as approved
    db.prepare(`
      UPDATE cancellation_requests
      SET status = ?, approved_by = ?, approved_at = ?
      WHERE id = ?
    `).run('approved', approvedBy, now, requestId);

    // Cancel order
    store.updateStatusAndSource(request.order_id, 'cancelled', { cancelledAt: now });

    let inventoryResult = null;
    if (inventory) {
      try { inventoryResult = inventory.releaseOrder(order, previousStatus); } catch (e) {
        store.updateSource(request.order_id, { inventoryError: e.message });
      }
    }
  ...
  async function rejectCancellation(requestId, reason = '', rejectedBy = 'system') {
    const request = store.db.prepare('SELECT * FROM cancellation_requests WHERE id = ?').get(requestId);
    if (!request) { const e = new Error('Cancellation request not found'); e.status = 404; throw e; }
    if (request.status !== 'pending') throw Object.assign(new Error('Cancellation not pending'), { status: 400 });
    ...
    db.prepare(`
      ...
      WHERE id = ?
    `).run('rejected', rejectedBy, now, reason, requestId);
  ...
  return { fulfill, scanFulfill, getWaybill, cancelOrder, returnOrder, requestCancellation, approveCancellation, rejectCancellation, NEXT_STATUS };
};
