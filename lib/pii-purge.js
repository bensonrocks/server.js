...
// `dryRun` counts what WOULD be redacted without touching anything — the
// preview an operator confirms against before an irreversible erase.
function redactFields(obj, fields, dryRun) {
  let changed = false;
  for (const f of fields) if (obj[f] && obj[f] !== PII_MARK) { if (!dryRun) obj[f] = PII_MARK; changed = true; }
  return changed;
}
...
function purgeBatches(batches, cutoffIso, keys, { dryRun = false } = {}) {
  let hits = 0;
  for (const b of batches || []) {
    for (const o of b.orders || []) {
      ...
      if (endT && endT < cutoffIso && redactFields(o, ORDER_PII, dryRun)) { if (!dryRun) o.pii_purged = true; hits++; }
    }
  }
  return hits;
}
...
function purgeAudit(events, cutoffIso, keys, { dryRun = false } = {}) {
  let hits = 0;
  for (const e of events || []) {
    ...
    if (isMkt && redactFields(e, AUDIT_PII, dryRun)) { if (!dryRun) e.pii_purged = true; hits++; }
  }
  return hits;
}
