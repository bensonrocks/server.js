...
const DEFAULT_USER = process.env.IDEAL_ADMIN_USERNAME || 'admin';
...
function getUsername(tenantDb) {
  const row = tenantDb.prepare("SELECT value FROM settings WHERE key='admin_username'").get();
  if (row) return row.value;
  tenantDb.prepare("INSERT INTO settings (key, value) VALUES ('admin_username', ?)").run(DEFAULT_USER);
  return DEFAULT_USER;
}

function checkUsername(tenantDb, username) {
  return username === getUsername(tenantDb);
}

function changeUsername(tenantDb, newUsername) {
  tenantDb.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('admin_username', ?)").run(newUsername);
}
...
module.exports = { checkPassword, checkUsername, getUsername, changeUsername, generateToken, validateToken, revokeToken, changePassword, revokeAllTenantSessions };
