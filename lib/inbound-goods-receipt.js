...
module.exports = function createInboundGoodsReceipt(db, inventoryWarehouse) {
  ...
  const qualityCheckItems = (inboundId, checks = []) => {
    ...
    for (const check of checks) {
      const {
        ...
        notes = '',
        inspectorName = '',
        photoBase64 = null     // Optional photo for this check
      } = check;

      // Log quality check
      const qcCheckId = require('crypto').randomUUID();
      db.prepare(`
      ...
      // Capture photo if provided
      let photoId = null;
      if (photoBase64) {
        try {
          const photoBuffer = Buffer.from(photoBase64.replace(/^data:image\/\w+;base64,/, ''), 'base64');
          photoId = require('crypto').randomUUID();
          db.prepare(`
            INSERT INTO inbound_photos (
              id, inbound_id, scan_id, qc_inspection_id, context,
              photo_data, filename, mime_type, size_bytes, captured_by, notes, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(
            photoId,
            inboundId,
            ...
            'damage',
            photoBuffer,
            `qc_check_${damageLevel}_${qcCheckId.slice(0, 8)}.jpg`,
            'image/jpeg',
            photoBuffer.length,
            inspectorName || 'staff',
            `QC damage level: ${damageLevel}`,
            now
          );
        } catch (photoErr) {
          // Log photo error but don't fail the QC check
          console.error('Photo capture error:', photoErr.message);
        }
      }
  ...
  const getReceivingMetrics = (warehouseId = 'wh-main', days = 7) => {
    ...
    const items = db.prepare(`
      SELECT
        ...
        SUM(inbound_scans.qty) as total_qty,
        COUNT(DISTINCT inbound_scans.inbound_id) as receipts,
        ROUND(SUM(inbound_scans.qty) / COUNT(DISTINCT inbound_scans.inbound_id), 1) as avg_qty_per_receipt,
        COUNT(CASE WHEN inbound_scans.status = 'quarantined' THEN 1 END) as quarantined_count,
        ROUND(COUNT(CASE WHEN inbound_scans.status = 'quarantined' THEN 1 END) * 100.0 / COUNT(*), 1) as quarantine_pct
      FROM inbound_scans
      JOIN inbound_receipts ON inbound_scans.inbound_id = inbound_receipts.id
   ...
   */
  const capturePhoto = (inboundId, photoData = {}) => {
    const {
      scanId = null,
      qcInspectionId = null,
      context = 'qc',  // qc, damage, condition, quarantine
      photoBase64 = '',
      filename = '',
      capturedBy = 'staff',
      notes = ''
    ...
    if (!photoBase64) {
      throw new Error('Photo data required');
    }
    ...
    const photoBuffer = Buffer.from(photoBase64.replace(/^data:image\/\w+;base64,/, ''), 'base64');
    const sizeBytes = photoBuffer.length;
    ...
    `).run(
      ...
      context,
      photoBuffer,
      filename || `photo_${photoId.slice(0, 8)}.jpg`,
      'image/jpeg',
      sizeBytes,
      capturedBy,
      notes,
    ...
    return {
      ...
      qcInspectionId,
      context,
      filename: filename || `photo_${photoId.slice(0, 8)}.jpg`,
      sizeBytes,
      capturedBy,
      capturedAt: now,
      notes
  ...
  /**
   * Get photos for an inbound/scan/QC inspection
   */
  const getPhotos = (inboundId, scanId = null, qcInspectionId = null, context = null) => {
    let sql = 'SELECT id, inbound_id, scan_id, qc_inspection_id, context, filename, size_bytes, captured_by, notes, created_at FROM inbound_photos WHERE inbound_id = ?';
    const params = [inboundId];
    ...
    if (qcInspectionId) {
      sql += ' AND qc_inspection_id = ?';
      params.push(qcInspectionId);
    }
    if (context) {
      sql += ' AND context = ?';
      params.push(context);
    ...
    return photos.map(p => ({
      photoId: p.id,
      inboundId: p.inbound_id,
      scanId: p.scan_id,
      qcInspectionId: p.qc_inspection_id,
      context: p.context,
      filename: p.filename,
  ...
  const getPhotoData = (photoId) => {
    const photo = db.prepare(`
      SELECT id, photo_data, filename, mime_type FROM inbound_photos WHERE id = ?
    `).get(photoId);
    ...
    return {
      photoId: photo.id,
      photoData: photo.photo_data ? photo.photo_data.toString('base64') : null,
  ...
  const deletePhoto = (photoId) => {
    const photo = db.prepare('SELECT id FROM inbound_photos WHERE id = ?').get(photoId);
    if (!photo) throw new Error('Photo not found');

    db.prepare('DELETE FROM inbound_photos WHERE id = ?').run(photoId);

    return { photoId, deleted: true, deletedAt: new Date().toISOString() };
  };

  /**
   * Get photo statistics for inbound receipt
   */
  const getPhotoStats = (inboundId) => {
    ...
    const total = db.prepare(`
      ...
      FROM inbound_photos WHERE inbound_id = ?
    `).get(inboundId);

    return {
      inboundId,
      totalPhotos: total.count || 0,
      totalBytes: total.total_bytes || 0,
      byContext: stats.map(s => ({
        context: s.context,
        count: s.count,
