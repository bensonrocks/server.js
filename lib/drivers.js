...
module.exports = function createDrivers({ db, store }) {
  ...
  function listDrivers() {
    ...
    const counts = db.prepare(
      `SELECT driver_id, COUNT(*) AS n FROM deliveries
       WHERE status IN ('assigned','picked_up','in_transit') GROUP BY driver_id`
    ).all();
    const countMap = Object.fromEntries(counts.map(c => [c.driver_id, c.n]));
  ...
  function createDriver({ name, phone, vehicle = '', plate = '', pin }) {
    ...
    db.prepare(
      'INSERT INTO drivers (id, name, phone, vehicle, plate, pin_hash) VALUES (?, ?, ?, ?, ?, ?)'
    ).run(id, name, phone, String(vehicle || '').trim(), String(plate || '').trim(), sha256(pin));
    return getDriver(id);
  }

  function updateDriver(id, { name, phone, vehicle, plate, pin, active } = {}) {
  ...
  function assign(driverId, orderIds, assignedBy = '') {
    ...
    const insert = db.prepare(
      'INSERT INTO deliveries (id, order_id, driver_id, assigned_by) VALUES (?, ?, ?, ?)'
    );
  ...
  function updateStatus(deliveryId, newStatus, {
    ...
    if (order) {
      if ((newStatus === 'picked_up' || newStatus === 'in_transit')
        ...
        store.updateStatusAndSource(order.id, 'shipped', { outForDeliveryAt: now });
        ...
        store.updateSource(order.id, { deliveryFailedAt: now, deliveryFailedReason: String(reason).trim() });
      }
    }
  ...
  return {
    ...
    assign, listDeliveries, getDelivery, updateStatus, unassign,
