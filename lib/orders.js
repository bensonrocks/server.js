...
function buildOrder(organizationId, createdByUserId, data) {
...
const json = {
  create(organizationId, createdByUserId, data) {
    const list = jsonRead();
    const order = buildOrder(organizationId, createdByUserId, data);
    list.push(order);
    jsonWrite(list);
    return order;
  },
  ...
  listByOrganization(organizationId) {
    return jsonRead().filter(o => o.organizationId === organizationId).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  },
  listAll() {
...
const pg = {
  async create(organizationId, createdByUserId, data) {
    const o = buildOrder(organizationId, createdByUserId, data);
    const { rows } = await pool.query(
      `INSERT INTO orders (
         id, organization_id, created_by_user_id, recipient_name, address_line1, city, region, postal_code, country,
       ...
       RETURNING *`,
      [
        o.id, o.organizationId, o.createdByUserId, o.recipientName, o.addressLine1, o.city, o.region, o.postalCode, o.country,
  ...
  async listByOrganization(organizationId) {
    const { rows } = await pool.query(
      'SELECT * FROM orders WHERE organization_id = $1 ORDER BY created_at DESC', [organizationId]
    );
