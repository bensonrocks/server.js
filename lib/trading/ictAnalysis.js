...
function buildTradePlan({ direction, zone, pipSize, swingTargets, fibZone, stopLossPips = STOP_LOSS_PIPS }) {
  const entry = (zone.low + zone.high) / 2;
  const slPips = stopLossPips;
...
// `params` optionally overrides the tunable defaults (used by the IDEAL
// engine, which adjusts them from graded signal outcomes):
//   minZonePips  — minimum buy/sell zone width
//   stopLossPips — stop-loss distance from entry
function analyze(candles, instrumentKey, params = {}) {
  const pipSize = PIP_SIZE[instrumentKey];
  if (!pipSize) throw new Error(`No pip size configured for ${instrumentKey}`);

  const minZonePips  = Number.isFinite(params.minZonePips)  ? params.minZonePips  : MIN_ZONE_PIPS;
  const stopLossPips = Number.isFinite(params.stopLossPips) ? params.stopLossPips : STOP_LOSS_PIPS;
  ...
  if (direction === 'buy') {
    ...
    const low = candidates.length ? Math.min(...candidates.map((c) => c.low ?? c.bottom)) : currentPrice - minZonePips * pipSize;
    const high = candidates.length ? Math.max(...candidates.map((c) => c.high ?? c.top)) : currentPrice;
    zone = buildZone(low, high, pipSize, minZonePips);
    ...
    const high = candidates.length ? Math.max(...candidates.map((c) => c.high ?? c.top)) : currentPrice + minZonePips * pipSize;
    zone = buildZone(low, high, pipSize, minZonePips);
    tradePlan = buildTradePlan({
      direction: 'sell',
      zone,
  ...
  return {
    ...
    pipSize,
    params: { minZonePips, stopLossPips },
    currentPrice,
