...
//     lesson applied before it is needed. Its request body is documented as
//     multipart/form-data (confirmed from the endpoint's own spec section,
//     not assumed from the general preamble) — the ONE call in this client
//     that is not plain JSON; `request()`'s `formData` option exists only
//     for this. Its own success body is `{print_jobs}` directly, no `data`
//     wrapper — `printAwbs()` falls back to the raw body when `.data` isn't
//     an object, same belt as every other reader in this file.
//
...
async function request(store, method, path, { query, body, formData } = {}) {
  ...
  // formData (a FormData instance) is for the one endpoint the spec documents
  // as multipart/form-data (print_awbs) — fetch sets the Content-Type itself,
  // boundary included; setting it by hand here would drop the boundary and
  // produce a body the server can't parse. Every other endpoint stays JSON,
  // which is what the rest of the spec documents.
  let fetchBody;
...
// POST /orders/print_awbs — generate shipping labels for a batch. THE ONE
// ENDPOINT THE SPEC DOCUMENTS AS multipart/form-data, unlike the rest of the
// API (which the spec's own preamble says takes query/form/JSON bodies
// interchangeably) — sent as JSON here until the user supplied the endpoint's
// own spec section confirming multipart specifically. Array fields in
// multipart are the same field name repeated once per value, the ordinary
// convention (no order_ids[] bracket suffix in the spec's field name).
async function printAwbs(store, orderIds, { withSkuList = false, documentType } = {}) {
  const fd = new FormData();
  for (const id of orderIds) fd.append('order_ids', String(Number(id)));
  if (withSkuList) fd.append('with_sku_list', 'true');
  if (documentType) fd.append('document_type', documentType);
  const { json } = await request(store, 'POST', '/orders/print_awbs', { formData: fd });
  return json.data && typeof json.data === 'object' ? json.data : json;
}
