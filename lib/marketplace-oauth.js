...
// ── Lazada ──────────────────────────────────────────────────────────────────
// VERIFIED AGAINST LAZADA'S OFFICIAL JAVA SDK (lazop-api-sdk-1.2.2 sources,
// supplied by the user): sign base = apiName + sorted(key+value) [+ body],
// HMAC-SHA256 with the app secret as a UTF-8 string, UPPERCASE hex; timestamp
// = epoch MILLISECONDS; auth/token calls go to https://auth.lazada.com/rest.
// All of which this file already implements — so an exchange failure is a
// PERMISSION problem, never a signing one.
// BUSINESS APIs (orders, logistics — for the future direct order pull) use
// PER-COUNTRY gateways, NOT the auth host: Singapore is
// https://api.lazada.sg/rest (MY: api.lazada.com.my, TH: api.lazada.co.th,
// VN: api.lazada.vn, PH: api.lazada.com.ph, ID: api.lazada.co.id).
const LAZADA_AUTH_BASE = 'https://auth.lazada.com';
const LAZADA_API_BASE  = 'https://auth.lazada.com/rest';
const LAZADA_GATEWAY_SG = 'https://api.lazada.sg/rest';   // business APIs, SG sellers
