...
function binLotsForSku(clientId, sku, strategy = 'fefo') {
  ...
  const order = `(CASE WHEN wl.kind='bulk' THEN 1 ELSE 0 END), ${rotation}`;
  return db.prepare(`SELECT bl.*, wl.zone, wl.aisle, wl.shelf, wl.bin, wl.kind FROM bin_lots bl
    LEFT JOIN warehouse_locations wl ON wl.location_id=bl.location_id
...
function suggestPutaway(clientId, sku, qty, opts = {}) {
  ...
  const bins = db.prepare(`SELECT wl.*,
  ...
  if (!bins.length) return { suggestions: [], notes: ['No bins are set up yet — add locations under Inventory first.'] };
  ...
  const scored = usable.map(b => {
    ...
    // Heavy goods low down. Shelf is a label, so this is a heuristic on its
    // ordering, said plainly rather than dressed up as a calculation.
    if (heavy) {
      const lowish = /^0*[12]$/.test(String(b.shelf || ''));
      if (lowish) { score += 25; reasons.push('low shelf for a heavy carton'); }
      else score -= 10;
    }
...
module.exports = {
  ...
  binLotsForSku, suggestLocationForSku, suggestSlotForNewSku, suggestPutaway, allocatePick, consumeAllocations, consumeFromLocation, restoreLots, zeroBinLot, caseBreakReplenish, binnedQty, stagingQty,
  // Serial numbers
