...
function upsertBundle(clientId, bundleSku, name, components) {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  if (!clientId) throw new Error('clientId is required');
  ...
  db.prepare(`INSERT INTO bundles (client_id, bundle_sku, name, components, updated_at)
    VALUES (?,?,?,?,datetime('now'))
    ON CONFLICT(client_id, bundle_sku) DO UPDATE SET name=excluded.name, components=excluded.components, updated_at=datetime('now')`)
    .run(clientId, sku, String(name || sku), JSON.stringify(comps));
...
function _rowToBundle(r) {
  ...
  return { client_id: r.client_id, bundle_sku: r.bundle_sku, name: r.name, components, updated_at: r.updated_at };
}
...
// Bundle SKUs (for this client) whose recipe includes the given component SKU —
// used to re-push a bundle's derived availability whenever a component moves.
function bundlesContaining(clientId, componentSku) {
  return getBundles(clientId).filter(b => b.components.some(c => c.sku === componentSku)).map(b => b.bundle_sku);
}
...
module.exports = {
  ...
  seedFromSkuMap,
  // Bundles / BOM
  upsertBundle, getBundle, getBundles, deleteBundle, bundleAvailable, bundlesContaining,
  // Warehouse locations
