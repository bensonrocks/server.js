...
function setStockPositions(clientId, rows, { operator = '', reason = 'stock position import' } = {}) {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  if (!clientId) throw new Error('clientId is required');
  ...
  db.transaction(() => {
    for (const r of rows || []) {
      ...
      // SET, not add: clear this SKU's lots at this bin, then write the figure.
    ...
    for (const sku of out.skus) {
      ...
      db.prepare(`INSERT INTO stock_movements (sku, client_id, type, qty, reason, operator)
                  VALUES (?,?,'adjustment',?,?,?)`).run(sku, clientId, n, reason, operator);
    }
  })();
  out.skus = [...out.skus];
  return out;
...
module.exports = {
  ...
  checkAvailability, outboundMovements, setStockPositions,
