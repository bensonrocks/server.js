...
function transferStock(clientId, sku, fromLocation, toLocation, qty, operator = '') {
  ...
  return db.transaction(() => {
    ...
    db.prepare('UPDATE stock_by_location SET quantity=quantity-?, last_counted=CURRENT_TIMESTAMP WHERE client_id=? AND sku=? AND location_id=?')
...
// Place stock into a bin (basic putaway / initial bin seeding). stock_by_location
// is otherwise only ever written by transferStock, which needs stock to already be
// somewhere — so without this, bins can never be seeded and transfers are unusable.
// This records WHERE units physically sit; it deliberately does NOT change the
// authoritative sellable total (inventory.stock_qty), exactly like transferStock —
// bin quantities are a physical-distribution view layered on top of the total.
function placeStock(clientId, sku, locationId, qty, operator = '') {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  if (!clientId) throw new Error('clientId is required');
  if (!sku || !locationId) throw new Error('sku and locationId required');
  const n = Number(qty);
  if (!Number.isFinite(n) || n === 0) throw new Error('qty must be a non-zero number');
  return db.transaction(() => {
    ...
    db.prepare(`INSERT INTO stock_by_location (client_id, sku, location_id, quantity, last_counted) VALUES (?,?,?,?,datetime('now'))
      ON CONFLICT(client_id, sku, location_id) DO UPDATE SET quantity=MAX(0, quantity+?), last_counted=datetime('now')`)
      .run(clientId, sku, locationId, Math.max(0, n), n);
    db.prepare('INSERT INTO stock_movements (sku, client_id, type, qty, reason, to_location, operator, at) VALUES (?,?,?,?,?,?,?,CURRENT_TIMESTAMP)')
      .run(sku, clientId, 'putaway', n, `Placed at ${locationId}`, locationId, operator);
    const row = db.prepare('SELECT quantity FROM stock_by_location WHERE client_id=? AND sku=? AND location_id=?').get(clientId, sku, locationId);
    return { ok: true, sku, location_id: locationId, quantity: row ? row.quantity : 0 };
  })();
}

// Client-wide bin occupancy — every SKU×bin with a non-zero quantity, joined to
// the product name and the bin's zone/aisle/shelf. Powers the Locations overview.
function locationStockForClient(clientId) {
  const db = _open();
  if (!db) return [];
  if (!clientId) throw new Error('clientId is required');
  return db.prepare(`SELECT sbl.sku, sbl.location_id, sbl.quantity, sbl.last_counted,
      wl.zone, wl.aisle, wl.shelf, wl.bin, wl.environment, i.name
    FROM stock_by_location sbl
    LEFT JOIN warehouse_locations wl ON wl.location_id=sbl.location_id
    LEFT JOIN inventory i ON i.client_id=sbl.client_id AND i.sku=sbl.sku
    WHERE sbl.client_id=? AND sbl.quantity>0
    ORDER BY wl.zone, wl.aisle, wl.shelf, wl.bin, sbl.sku`).all(clientId);
}
...
module.exports = {
  ...
  // Warehouse locations
  createLocation, getLocations, stockByLocation, transferStock, placeStock, locationStockForClient,
  // Suppliers
