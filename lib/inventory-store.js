...
function setStockPositions(clientId, rows, { operator = '', reason = 'stock position import', mode = 'add' } = {}) {
  ...
  const out = { placed: 0, units: 0, skusCreated: 0, binsCreated: 0, skipped: [], short: [], skus: new Set(),
                zeroed: [], zeroedUnits: 0, untouched: [] };
  ...
  db.transaction(() => {
    ...
    if (mode === 'set') {
      // A SKU is only "in the file" for the purposes of a stock-take if the
      // file gives us a row we can actually PLACE — a SKU, a location and a
      // quantity. A row naming a SKU with a BLANK location cannot be placed,
      // and the first cut cleared that SKU's bins anyway while leaving its
      // on-hand standing: bins gone, 200 pcs still on the books, and a preview
      // promising zero. Three disagreeing answers from one upload.
      //
      // So a SKU the file mentions but cannot place is left COMPLETELY alone —
      // bins and on-hand both — and REPORTED. Zeroing it would destroy real
      // stock over a blank cell; half-clearing it strands the SKU. Neither is
      // something to do silently, and the operator fixes the row and re-runs.
      const placeable = new Set((rows || [])
        .filter(r => String(r.sku || '').trim() && String(r.location || '').trim()
                     && Number.isFinite(Number(r.qty)) && Number(r.qty) >= 0)
        .map(r => String(r.sku).trim()));
      const mentioned = new Set((rows || []).map(r => String(r.sku || '').trim()).filter(Boolean));
      ...
      const inFile = placeable;
      for (const sku of held) {
        if (mentioned.has(sku) && !placeable.has(sku)) {
          const it = get(sku, clientId);
          out.untouched.push({ sku, kept: it ? Number(it.stock_qty) || 0 : 0 });
          continue;                       // named but unplaceable — leave as is
        }
...
function previewStockPositions(clientId, rows, mode = 'add') {
  ...
  for (const r of rows || []) {
    ...
    const loc = String(r.location || '').trim().toUpperCase();
    const qty = Number(r.qty);
    if (!sku || !loc || !Number.isFinite(qty) || qty < 0) {
      out.errors.push({ sku, location: loc, reason: 'missing sku, location or quantity' }); continue;
    }
    const item = get(sku, clientId);
  ...
  if (mode === 'set') {
    const inFile = new Set(out.skus);          // SKUs with a PLACEABLE row
    ...
    // A SKU the file NAMES but cannot place (a blank location, say) is left
    // exactly as it is by the write — so it is NOT going to zero, and its stock
    // is still there afterwards. Counting it as zeroed made the preview promise
    // a total the upload would never reach, which is the one thing a confirm
    // must never do.
    const named = new Set((rows || []).map(r => String(r.sku || '').trim()).filter(Boolean));
    out.untouchedSkus = rowsNow
      .filter(r => named.has(r.sku) && !inFile.has(r.sku))
      .map(r => ({ sku: r.sku, keeps: r.n }));
    out.untouchedUnits = out.untouchedSkus.reduce((n, r) => n + r.keeps, 0);
    out.afterTotal   = out.units + out.untouchedUnits;
    out.zeroSkus = rowsNow.filter(r => !named.has(r.sku)).map(r => ({ sku: r.sku, was: r.n }));
    out.zeroUnits = out.zeroSkus.reduce((n, r) => n + r.was, 0);
    // Stock on a SKU the file DOES list, but in bins it does not name, is
    // replaced too — counted here so the arithmetic on screen adds up.
    out.replacedUnits = out.currentTotal - out.zeroUnits - out.untouchedUnits;
  }
  return out;
}
