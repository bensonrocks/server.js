...
function ledgerUploads(clientId, { days = 30 } = {}) {
  ...
  const rows = db.prepare(`SELECT substr(at,1,16) AS minute, COUNT(*) AS skus,
      SUM(qty) AS units, MIN(at) AS from_at, MAX(at) AS to_at
    FROM stock_movements
    WHERE client_id=? AND type='upload' AND at >= datetime('now', ?)
    GROUP BY substr(at,1,16) ORDER BY minute DESC`).all(clientId, `-${Number(days) || 30} days`);
  return rows.map(r => ({ ...r, id: `${clientId}|${r.minute}` }));
}

// Give back exactly what that minute's upload rows added. Each is undone with
// its own compensating movement, so the ledger keeps both halves of the story
// rather than pretending the upload never happened.
function reverseLedgerUpload(clientId, minute, { operator = '', reason = 'stock upload undone' } = {}) {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  if (!clientId || !minute) throw new Error('clientId and minute are required');
  const moves = db.prepare(`SELECT sku, SUM(qty) AS qty FROM stock_movements
    WHERE client_id=? AND type='upload' AND substr(at,1,16)=? GROUP BY sku`).all(clientId, minute);
  if (!moves.length) throw new Error('No upload movements found for that time');
  const out = { skus: 0, units: 0, floored: [], created: [] };
  db.transaction(() => {
    for (const m of moves) {
      ...
      // Never below zero, and say so when the shelf no longer holds what the
      // upload put there — something has shipped since, and that is real.
  ...
  // SKUs first seen in that same minute carry no movement row — the upload
  // created them. Named, never silently zeroed.
  out.created = db.prepare(`SELECT sku, stock_qty FROM inventory
    ...
    .all(clientId, minute, clientId, minute);
  return out;
}
