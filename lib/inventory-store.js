...
function bulkUpsertLocations(rows) {
  ...
  const tx = db.transaction(list => {
    list.forEach((raw, i) => {
      ...
      if (!zone || !aisle || !shelf || !bin) {
        result.skipped++;
        result.errors.push({ row: rowNo, error: 'zone, aisle, shelf and bin are all required' });
        return;
      }
      const id  = `${zone}-${aisle}-${shelf}-${bin}`;
...
module.exports = {
  ...
  warehouseOverview,
  createLocation, getLocations, getLocation, updateLocation, bulkUpsertLocations, generateLocations, locationMap, locationOccupancy, binCapacityCheck, binContents, replenishmentSuggestions, stockByLocation,
  transferStock, placeStock, locationStockForClient,
  binLotsForSku, suggestLocationForSku, suggestSlotForNewSku, allocatePick, consumeAllocations, consumeFromLocation, restoreLots, zeroBinLot, caseBreakReplenish, binnedQty, stagingQty,
  // Serial numbers
