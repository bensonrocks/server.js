...
const PRODUCT_MASTER_COLUMNS = [
  ...
  // Written by harvestCatalogueFromOrders when it CREATES a row from an order
  // that passed through, rather than from a catalogue anyone loaded. Such a
  // row proves nothing about ownership — see buildSkuOwnerIndex, which must
  // not let it make a SKU look like it belongs to two clients.
  ['learned_from_orders', 'INTEGER', 0],
];
...
function adjust(sku, clientId, qty, type = 'adjustment', reason = '', orderId = null) {
  ...
  // ONE LEDGER: a downward adjustment can leave bins claiming units that just
  // left the books (a write-off of binned stock). Cap the bins at the new
  // on-hand so a pick list never points at stock that is not there.
  if (Number(qty) < 0) {
    try { enforceBinCap(clientId, sku, { reason: `Follows: ${reason || type}` }); } catch (_) {}
  }
  return get(sku, clientId);
}
...
// WHERE IS THE STOCK ACTUALLY BINNED for this product? Given an order's SKU
// and/or barcode, return the EXACT bin-lot SKU (with stock on the shelf) that
// belongs to this product, or '' if none. The order's code may differ from
// the binned code in three ways this bridges: casing (SQLite `=` is
// case-sensitive, so a putaway under a different case silently misses), a
// shared barcode (a marketplace variant vs the in-house SKU), or the
// catalogue's own SKU. Returns the stored spelling so the exact allocatePick
// that follows still matches.
function resolveBinnedSku(clientId, { sku, barcode } = {}) {
  const db = _open();
  if (!db || !clientId) return '';
  const s = String(sku || '').trim();
  const bc = String(barcode || '').trim();
  // 1. exact SKU with stock.
  if (s && binnedQty(clientId, s) > 0) return s;
  // 2. case-insensitive SKU with stock — the exact stored spelling.
  if (s) {
    const r = db.prepare("SELECT sku, COALESCE(SUM(qty),0) AS n FROM bin_lots WHERE client_id=? AND LOWER(TRIM(sku))=LOWER(?) GROUP BY sku HAVING n>0 ORDER BY n DESC LIMIT 1").get(clientId, s);
    if (r && r.sku) return r.sku;
  }
  // 3. any SKU sharing the barcode that has stock. Join bin_lots to inventory
  //    on the barcode so a variant SKU on the order lands on the stocked SKU.
  if (bc) {
    const r = db.prepare(`SELECT bl.sku, COALESCE(SUM(bl.qty),0) AS n
      FROM bin_lots bl JOIN inventory i ON i.client_id=bl.client_id AND i.sku=bl.sku
      WHERE bl.client_id=? AND i.barcode<>'' AND LOWER(TRIM(i.barcode))=LOWER(?)
      GROUP BY bl.sku HAVING n>0 ORDER BY n DESC LIMIT 1`).get(clientId, bc);
    if (r && r.sku) return r.sku;
  }
  return '';
}
// THE FULL BRIDGE for pick lists: where is this product's stock ACTUALLY
// binned, folding BOTH identities the rest of the app already treats as
// case-insensitive — the SKU and the CLIENT. Reported live: bins recorded and
// visible on the Bin Locations screen (client keyed "Mayer2026") while the
// wave resolved its client from the batch ("MAYER2026") — same client, one
// case apart, and the exact `client_id=?` match missed every bin, so the pick
// list showed no location for stock the system plainly held. Case-folding the
// client here is the standing "one client, one spelling" rule, not a loosening
// — it can only unite case-variants, never bridge two different names.
// Returns { client_id, sku } in the EXACT stored spellings (so the exact-match
// allocatePick that follows finds the same rows), or null.
function resolveBinnedFor(clientId, { sku, barcode } = {}) {
  const db = _open();
  if (!db || !clientId) return null;
  const cid = String(clientId).trim();
  const s = String(sku || '').trim();
  const bc = String(barcode || '').trim();
  // 1. exact client + exact SKU — the fast path, and the common case.
  if (s && binnedQty(cid, s) > 0) return { client_id: cid, sku: s };
  // 2. case-insensitive client + SKU — the exact stored spellings win.
  if (s) {
    const r = db.prepare(`SELECT client_id, sku, COALESCE(SUM(qty),0) AS n FROM bin_lots
      WHERE LOWER(TRIM(client_id))=LOWER(?) AND LOWER(TRIM(sku))=LOWER(?)
      GROUP BY client_id, sku HAVING n>0 ORDER BY n DESC LIMIT 1`).get(cid, s);
    if (r && r.sku) return { client_id: r.client_id, sku: r.sku };
  }
  // 3. any SKU sharing the barcode with stock, client case-folded on both sides.
  if (bc) {
    const r = db.prepare(`SELECT bl.client_id, bl.sku, COALESCE(SUM(bl.qty),0) AS n
      FROM bin_lots bl JOIN inventory i ON LOWER(TRIM(i.client_id))=LOWER(TRIM(bl.client_id)) AND i.sku=bl.sku
      WHERE LOWER(TRIM(bl.client_id))=LOWER(?) AND i.barcode<>'' AND LOWER(TRIM(i.barcode))=LOWER(?)
      GROUP BY bl.client_id, bl.sku HAVING n>0 ORDER BY n DESC LIMIT 1`).get(cid, bc);
    if (r && r.sku) return { client_id: r.client_id, sku: r.sku };
  }
  return null;
}
// EVERY SKU that carries this barcode. A marketplace order can reference a
// variant SKU (FS1605XXXXBKML) while the stock is binned under the in-house
// SKU (FS1610DRXXBKML); if both catalogue rows share the barcode, this is the
// bridge the pick list needs to find where the stock actually is. Returns
// SKUs newest-binned first so a caller wanting "the one that has stock" can
// pick the first with binnedQty>0. Case/space-insensitive on the barcode.
function skusForBarcode(clientId, barcode) {
  const db = _open();
  if (!db || !clientId) return [];
  const bc = String(barcode || '').trim();
  if (!bc) return [];
  return db.prepare("SELECT sku FROM inventory WHERE client_id=? AND barcode <> '' AND LOWER(TRIM(barcode)) = LOWER(?)")
    .all(clientId, bc).map(r => r.sku);
}
// ── ONE LEDGER: binned can never exceed on-hand ─────────────────────────────
// bin_lots is the BREAKDOWN of on-hand (where it sits), never a second total.
// Physically, a bin cannot hold units that are not on the books — when on-hand
// falls below what the bins claim (a hand adjustment down, a damage write-off,
// or a completion whose bin consumption missed under the old case-sensitive
// match), the bin records are stale and the pick list would send a picker to
// stock that has left. Trim the newest lots first (dated/older lots are the
// FEFO-relevant ones and the most likely to be real) down to on-hand, and put
// the trim on the movement ledger so the correction explains itself.
function enforceBinCap(clientId, sku, { operator = '', reason = '' } = {}) {
  const db = _open();
  if (!db || !clientId || !sku) return null;
  const item = get(sku, clientId);
  // A SKU no longer in the master with lots lingering is orphaned data — its
  // on-hand is definitionally zero, so the lots go too (same rule as the
  // client-wipe: item master implies stock).
  const onHand = item ? Number(item.stock_qty) || 0 : 0;
  const binned = binnedQty(clientId, sku);
  if (binned <= onHand) return null;
  let excess = binned - onHand;
  const trimmed = [];
  db.transaction(() => {
    const lots = db.prepare('SELECT id, location_id, qty FROM bin_lots WHERE client_id=? AND sku=? AND qty>0 ORDER BY id DESC').all(clientId, sku);
    for (const l of lots) {
      if (excess <= 0) break;
      const take = Math.min(excess, l.qty);
      db.prepare('UPDATE bin_lots SET qty=qty-? WHERE id=?').run(take, l.id);
      trimmed.push({ location_id: l.location_id, qty: take });
      excess -= take;
    }
    db.prepare('DELETE FROM bin_lots WHERE client_id=? AND sku=? AND qty<=0').run(clientId, sku);
    const units = trimmed.reduce((n, t) => n + t.qty, 0);
    if (units > 0) {
      db.prepare('INSERT INTO stock_movements (sku,client_id,type,qty,reason,operator) VALUES (?,?,?,?,?,?)')
        .run(sku, clientId, 'adjustment', -units,
             (reason || 'Bin records exceeded on-hand — trimmed to match') + ' (' + trimmed.map(t => `${t.location_id} −${t.qty}`).join(', ') + ')',
             operator);
    }
  })();
  const units = trimmed.reduce((n, t) => n + t.qty, 0);
  return units > 0 ? { client_id: clientId, sku, units, trimmed } : null;
}
// Sweep the whole store for binned > on-hand. REPORT-ONLY by default — a
// static overage found at rest is AMBIGUOUS: the bins may be the truth (stock
// binned by hand without a receipt behind it) and on-hand the error, and
// destroying either side unasked would erase a location somebody deliberately
// recorded (caught by the e2e before this ever shipped trimming at boot: the
// sweep ate 30 legitimately-binned units whose on-hand was simply never set).
// Event-driven trims (the adjust hook) stay automatic, because there the
// on-hand drop is a known event and the bins demonstrably went stale.
// `{ apply: true }` exists for an explicit, human-invoked reconcile.
function reconcileBinOverage({ apply = false } = {}) {
  const db = _open();
  if (!db) return { found: [], fixed: [], units: 0 };
  const rows = db.prepare(`SELECT bl.client_id, bl.sku, COALESCE(SUM(bl.qty),0) AS binned, COALESCE(i.stock_qty,0) AS on_hand
    FROM bin_lots bl LEFT JOIN inventory i ON i.client_id=bl.client_id AND i.sku=bl.sku
    WHERE bl.qty > 0
    GROUP BY bl.client_id, bl.sku HAVING binned > on_hand`).all();
  const found = rows.map(r => ({ client_id: r.client_id, sku: r.sku, binned: r.binned, on_hand: r.on_hand, over: r.binned - r.on_hand }));
  const fixed = [];
  if (apply) {
    for (const r of found) {
      try {
        const out = enforceBinCap(r.client_id, r.sku, { reason: 'Reconcile: bin records exceeded on-hand' });
        if (out) fixed.push(out);
      } catch (_) {}
    }
  }
  return { found, fixed, units: fixed.reduce((n, f) => n + f.units, 0), overUnits: found.reduce((n, f) => n + f.over, 0) };
}
// WHERE EVERY SKU SITS, in one query. Returns Map(sku -> [{location_id, qty}])
// for every SKU with binned stock, biggest bin first. Feeds the Location column
// on the Stock & SKUs list so a SKU showing no bin here is visibly "not put
// away" — the exact case that leaves a wave-pick location blank.
function binLocationsBySku(clientId) {
  const db = _open();
  const out = new Map();
  if (!db || !clientId) return out;
  const rows = db.prepare(
    "SELECT sku, location_id, COALESCE(SUM(qty),0) AS qty FROM bin_lots WHERE client_id=? AND qty>0 GROUP BY sku, location_id ORDER BY qty DESC"
  ).all(clientId);
  for (const r of rows) {
    if (!out.has(r.sku)) out.set(r.sku, []);
    out.get(r.sku).push({ location_id: r.location_id, qty: r.qty });
  }
  return out;
}
...
function setStockPositions(clientId, rows, { operator = '', reason = 'stock position import', mode = 'add' } = {}) {
  ...
  if (!db) throw new Error('inventory unavailable');
  if (!clientId) throw new Error('clientId is required');
  const out = { placed: 0, units: 0, skusCreated: 0, binsCreated: 0, skipped: [], short: [], skus: new Set(),
                zeroed: [], zeroedUnits: 0, untouched: [], unbinned: [] };
  // sku -> pieces the file counted with NO location (supersede only). Held on
  // the books as on-hand with no bin, which is exactly what the sheet said.
  const unbinned = new Map();
  // BEFORE-SNAPSHOT for the reversal window: every (sku, bin) cell the file
  // touches has its lot rows captured before the first touch, and every SKU
  // its on-hand figure — so "reverse" can put the exact position back rather
  // than guessing at deltas. Kept small: only what the file touches.
  const snapCells = new Map();   // "sku|loc" -> lots[]
  const snapSkus  = new Map();   // sku -> before_stock (null = SKU did not exist)
  // Cells this import has already cleared (supersede). A cell is emptied ONCE;
  // every later row naming it adds to it — see the note at the write below.
  const wroteCell = new Set();
  const snapCell = (sku, loc) => {
    const k = `${sku}|${loc}`;
    if (snapCells.has(k)) return;
    snapCells.set(k, db.prepare(`SELECT qty, lot_number, expiry_date, received_at FROM bin_lots
      WHERE client_id=? AND sku=? AND location_id=?`).all(clientId, sku, loc));
  };
  ...
  const insLoc  = db.prepare(`INSERT INTO warehouse_locations (location_id, zone, aisle, shelf, bin, capacity, environment, kind)
                              VALUES (?,?,?,?,?,?,?,?)`);
  db.transaction(() => {
    // ── SUPERSEDE MEANS THE FILE IS THE POSITION ─────────────────────────────
    // Reported live: 740 pcs uploaded as a supersede, and the screen still read
    // 844 — the previous balance was still in there. The old `set` cleared only
    // the (sku, bin) CELLS the file happened to name, so stock in any other bin,
    // and every SKU the file did not mention, survived untouched. That is not
    // superseding anything; it is a per-cell overwrite wearing the word.
    //
    // A supersede is a stock-take: after it, the client's position IS what the
    // sheet says. So every SKU the client holds is cleared first — those in the
    // file are then filled from it, and those absent from it go to ZERO and are
    // REPORTED by name, because a count that quietly leaves stock behind is
    // worse than no count at all.
    //
    // Everything cleared is snapshotted first, so the 3-day reversal can put
    // the whole position back and not just the cells the file named.
    if (mode === 'set') {
      // ── THE FILE IS THE POSITION. NO ARITHMETIC AT THE BACK END. ───────────
      // Per the user, after a round trip that proved the point: "just supersede
      // with whatever file I agree to use, with no further calculations at the
      // back end." So after this runs, on-hand for this client IS the sum of
      // the sheet — every time, with nothing to reconcile in anyone's head.
      //
      // What that settles: a row with a SKU and a QUANTITY but a BLANK Location
      // still sets that SKU's on-hand to the file's figure, simply UNBINNED.
      // (Their real file has 3 such rows — "209 SKU(s) · 206 located".) An
      // earlier cut left those SKUs completely alone, which was safe but meant
      // the total came out as neither the old position nor the file's, and the
      // operator had to work out why. "On hand but not in a bin" is a state
      // this system already models and already explains on screen, so the
      // honest answer is to record the count and say the location is missing —
      // not to refuse the count because the location is.
      //
      // So: EVERY SKU with a usable quantity is "in the file". Everything the
      // client holds is cleared first; file rows re-fill it (binned where a
      // location is given, unbinned where it is not); SKUs absent from the file
      // go to ZERO and are reported by name.
      const mentioned = new Set((rows || [])
        .filter(r => String(r.sku || '').trim()
                     && Number.isFinite(Number(r.qty)) && Number(r.qty) >= 0)
        .map(r => String(r.sku).trim()));
      const held = db.prepare(
        `SELECT sku FROM inventory WHERE client_id=? AND IFNULL(stock_qty,0) > 0
         UNION SELECT DISTINCT sku FROM bin_lots WHERE client_id=? AND qty > 0`
      ).all(clientId, clientId).map(r => r.sku);
      const inFile = mentioned;
      for (const sku of held) {
        for (const c of db.prepare('SELECT DISTINCT location_id AS loc FROM bin_lots WHERE client_id=? AND sku=?').all(clientId, sku)) {
          snapCell(sku, c.loc);
        }
        if (!snapSkus.has(sku)) {
          const it = get(sku, clientId);
          snapSkus.set(sku, it ? Number(it.stock_qty) || 0 : null);
        }
        db.prepare('DELETE FROM bin_lots WHERE client_id=? AND sku=?').run(clientId, sku);
        if (!inFile.has(sku)) {
          const had = Number(snapSkus.get(sku)) || 0;
          db.prepare('UPDATE inventory SET stock_qty=0 WHERE client_id=? AND sku=?').run(clientId, sku);
          db.prepare(`INSERT INTO stock_movements (sku, client_id, type, qty, reason, operator)
                      VALUES (?,?,?,?,?,?)`)
            .run(sku, clientId, 'adjustment', 0, `${reason} — not in the file, superseded to zero`, operator);
          out.zeroed.push({ sku, was: had });
          out.zeroedUnits += had;
        }
      }
    }
    for (const r of rows || []) {
      ...
      if (!sku || !Number.isFinite(qty) || qty < 0) { out.skipped.push({ sku, location: loc, reason: 'missing sku or quantity' }); continue; }
      // A COUNT WITH NO LOCATION IS STILL A COUNT (supersede only). The sheet
      // says this client holds `qty` of this SKU; that it does not say WHERE is
      // a separate, visible gap — the SKU reads "— not binned" on the stock
      // list and carries no pick location until someone locates it. Refusing
      // the count because the location is missing would leave the position
      // wrong, which is the worse of the two.
      if (!loc) {
        if (mode !== 'set') { out.skipped.push({ sku, location: '', reason: 'no location — cannot add to or reduce a bin' }); continue; }
        let it0 = get(sku, clientId);
        if (!snapSkus.has(sku)) snapSkus.set(sku, it0 ? Number(it0.stock_qty) || 0 : null);
        if (!it0) { upsert({ sku, clientId, name: r.name || sku, barcode: r.barcode || '', uom: r.uom || '' }); out.skusCreated++; }
        else if ((r.barcode && !it0.barcode) || (r.name && !it0.name)) {
          upsert({ ...it0, sku, clientId, name: it0.name || r.name, barcode: it0.barcode || r.barcode });
        }
        unbinned.set(sku, (unbinned.get(sku) || 0) + qty);
        out.skus.add(sku);
        continue;
      }

      if (!findLoc.get(loc)) {
        if (mode === 'reduce') { out.skipped.push({ sku, location: loc, reason: 'bin does not exist — nothing to reduce' }); continue; }
      ...
      if (!item && mode === 'reduce') {
        // Reducing a SKU the client does not have is a file error, never a
        // silent create-then-subtract-from-zero.
        out.skipped.push({ sku, location: loc, reason: 'SKU not in the item master — nothing to reduce' });
        continue;
      }
      if (!snapSkus.has(sku)) snapSkus.set(sku, item ? Number(item.stock_qty) || 0 : null);
      snapCell(sku, loc);
      if (!item) {
      ...
      // SET (supersede) exists for a stock-take, where the sheet states the
      // position rather than adding to it. REDUCE subtracts the sheet's
      // quantities — a write-off or correction list — flooring at zero and
      // REPORTING any shortfall rather than pretending the pieces were there.
      if (mode === 'set') {
        // A CELL IS CLEARED ONCE PER IMPORT, THEN ONLY ADDED TO. Reported live
        // as the last surviving piece of back-end arithmetic: the preview
        // promised 1,244 and the screen read 1,215. This DELETE ran before
        // EVERY row, so two rows naming the same SKU in the same bin — two
        // batches on one pallet, which is an ordinary way to write a stock
        // sheet — meant the second wiped the first and only the last figure
        // survived. (Rows in DIFFERENT bins were fine, which is why 208 rows
        // over 166 SKUs still looked plausible.) The file is the position, so
        // two rows for one cell SUM, exactly as two rows for one SKU do.
        const cellKey = `${sku}|${loc}`;
        if (!wroteCell.has(cellKey)) {
          db.prepare('DELETE FROM bin_lots WHERE client_id=? AND sku=? AND location_id=?').run(clientId, sku, loc);
          wroteCell.add(cellKey);
        }
        if (qty > 0) {
          // Same lot tops up its own row; a different lot gets its own, so a
          // sheet carrying real batch numbers still reads back per batch.
        ...
        if (qty > 0) {
          const lots = db.prepare(`SELECT id, qty FROM bin_lots
            WHERE client_id=? AND sku=? AND location_id=? AND qty > 0 ORDER BY id`).all(clientId, sku, loc);
          const had = lots.reduce((n, l) => n + l.qty, 0);
          let left = qty;
          for (const l of lots) {
            if (left <= 0) break;
            const take = Math.min(left, l.qty);
            db.prepare('UPDATE bin_lots SET qty=? WHERE id=?').run(l.qty - take, l.id);
            left -= take;
          }
          if (left > 0) out.short.push({ sku, location: loc, wanted: qty, had });
        }
      } else if (qty > 0) {
    ...
    for (const sku of out.skus) {
      const binnedN = db.prepare('SELECT COALESCE(SUM(qty),0) AS n FROM bin_lots WHERE client_id=? AND sku=?').get(clientId, sku).n;
      // On-hand = what sits in bins PLUS what the sheet counted without giving a
      // location. Reading the bins alone would silently zero every unlocated
      // row and the total would stop matching the file.
      const loose = unbinned.get(sku) || 0;
      const n = binnedN + loose;
      db.prepare('UPDATE inventory SET stock_qty=? WHERE client_id=? AND sku=?').run(n, clientId, sku);
      db.prepare(`INSERT INTO stock_movements (sku, client_id, type, qty, reason, operator)
                  VALUES (?,?,?,?,?,?)`).run(sku, clientId, mode === 'add' ? 'inbound' : 'adjustment', n, reason, operator);
      if (loose) out.unbinned.push({ sku, qty: loose });
    }
  })();
  out.skus = [...out.skus];
  // The snapshot travels with the result so the caller can store it against a
  // reversal window. asOfMovementId marks where the ledger stood WHEN THE
  // IMPORT FINISHED — anything written after it means the stock has since
  // moved, which is what decides whether a reversal is still clean.
  out.snapshot = {
    at: new Date().toISOString(),
    cells: [...snapCells.entries()].map(([k, lots]) => {
      const i = k.indexOf('|');
      return { sku: k.slice(0, i), location: k.slice(i + 1), lots };
    }),
    skus: [...snapSkus.entries()].map(([sku, before_stock]) => ({ sku, before_stock })),
    asOfMovementId: db.prepare('SELECT COALESCE(MAX(id),0) AS n FROM stock_movements').get().n,
  };
  return out;
}

// RECORD WHERE EXISTING STOCK SITS — locations only, quantities NEVER touched.
// The case this exists for: stock was uploaded through the plain stock-file
// uploader, which sets on-hand but DROPS the Location column, so a SKU reads
// "Stock OK" (on-hand > 0) yet has no bin_lot — and the wave pick shows no
// location, because the location was never recorded. This bins the on-hand
// ALREADY on the books at the locations named in the sheet. It is not a
// receipt and moves no total: on-hand is left exactly as it is, and nothing is
// ever binned beyond what the SKU already holds (so it can never inflate
// stock). A SKU already fully binned is skipped; one not in the item master is
// reported, never created (that would be inventing stock).
//
// `supersede` — the sheet REPLACES the locations of the SKUs it names. Every
// bin those SKUs currently occupy is cleared first, then their on-hand is
// binned where the sheet says, so a SKU genuinely MOVES from AA-001 to AA-002
// instead of being skipped as "already located". Deliberately scoped to the
// SKUs IN THE SHEET and to LOCATIONS ONLY: quantities are still never touched,
// and a SKU the sheet does not mention keeps its bins. Replacing a client's
// WHOLE position (zeroing absent SKUs) is Mass SUPERSEDE's job over in Putaway
// → Put away by file — two different promises, kept apart on purpose.
function locateExistingStock(clientId, rows, { operator = '', supersede = false } = {}) {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  if (!clientId) throw new Error('clientId is required');
  const out = { located: 0, units: 0, binsCreated: 0, notInMaster: [], alreadyLocated: [], overflow: [], cleared: [], skus: new Set() };
  // BEFORE-state of every (sku, bin) cell this touches, captured ONCE at its
  // true prior value. Clearing a bin is destructive, so the 3-day reversal has
  // to cover it — a snapshot with a hole exactly where the behaviour is
  // destructive is worse than no snapshot.
  const snapCells = new Map();
  const snapCell = (sku, loc) => {
    const k = sku + '|' + loc;
    if (snapCells.has(k)) return;
    snapCells.set(k, db.prepare(`SELECT qty, lot_number, expiry_date, received_at
      FROM bin_lots WHERE client_id=? AND sku=? AND location_id=?`).all(clientId, sku, loc));
  };
  const findLoc = db.prepare('SELECT location_id FROM warehouse_locations WHERE location_id=?');
  const insLoc  = db.prepare(`INSERT INTO warehouse_locations (location_id, zone, aisle, shelf, bin, capacity, environment, kind)
                              VALUES (?,?,?,?,?,?,?,?)`);
  // Remaining room per SKU = on-hand − already binned. We bin the UNBINNED
  // portion of on-hand and no more.
  const room = new Map();
  const roomFor = sku => {
    if (room.has(sku)) return room.get(sku);
    const it = get(sku, clientId);
    if (!it) { room.set(sku, null); return null; }        // not in master
    const onHand = Number(it.stock_qty) || 0;
    const binned = db.prepare('SELECT COALESCE(SUM(qty),0) AS n FROM bin_lots WHERE client_id=? AND sku=?').get(clientId, sku).n;
    const r = Math.max(0, onHand - binned);
    room.set(sku, r);
    return r;
  };
  db.transaction(() => {
    // SUPERSEDE: clear first, so `roomFor` below sees the SKU as unbinned and
    // its full on-hand goes where the sheet says. Only SKUs the sheet names
    // WITH a location, and only ones the item master actually carries — an
    // unknown SKU is reported, never touched.
    if (supersede) {
      const named = [...new Set((rows || [])
        .filter(r => String(r.sku || '').trim() && String(r.location || '').trim())
        .map(r => String(r.sku).trim()))];
      for (const sku of named) {
        if (!get(sku, clientId)) continue;                     // not in master
        const at = db.prepare('SELECT DISTINCT location_id FROM bin_lots WHERE client_id=? AND sku=?').all(clientId, sku);
        if (!at.length) continue;
        for (const c of at) snapCell(sku, c.location_id);
        const units = db.prepare('SELECT COALESCE(SUM(qty),0) AS n FROM bin_lots WHERE client_id=? AND sku=?').get(clientId, sku).n;
        db.prepare('DELETE FROM bin_lots WHERE client_id=? AND sku=?').run(clientId, sku);
        db.prepare(`INSERT INTO stock_movements (sku, client_id, type, qty, reason, from_location, operator)
                    VALUES (?,?,?,?,?,?,?)`)
          .run(sku, clientId, 'putaway', -units, 'Locations superseded by upload (no qty change)',
               at.map(x => x.location_id).join(','), operator);
        out.cleared.push({ sku, units, from: at.map(x => x.location_id) });
      }
    }
    for (const r of rows || []) {
      const sku = String(r.sku || '').trim();
      ...
      const qty = Number(r.qty);
      if (!sku || !loc) continue;
      const rm = roomFor(sku);
      if (rm === null) { out.notInMaster.push(sku); continue; }
      if (room.get(sku) <= 0) { out.alreadyLocated.push(sku); continue; }
      snapCell(sku, loc);
      // Cap at what is left of on-hand; a sheet with no quantity bins whatever
      // room remains into the named bin.
      const want = (Number.isFinite(qty) && qty > 0) ? qty : room.get(sku);
      const take = Math.min(want, room.get(sku));
      if (take <= 0) continue;
      if (!findLoc.get(loc)) {
        const p = parseLocationId(loc);
        insLoc.run(loc, p.row || 'IMPORT', p.bay || '', String(p.level || ''), p.pos || '', 100000, 'dry', 'pick');
      ...
      const lot = db.prepare(`SELECT id, qty FROM bin_lots
        WHERE client_id=? AND sku=? AND location_id=? AND expiry_date IS NULL AND IFNULL(lot_number,'')='' ORDER BY id LIMIT 1`).get(clientId, sku, loc);
      if (lot) db.prepare('UPDATE bin_lots SET qty=? WHERE id=?').run(lot.qty + take, lot.id);
      else db.prepare(`INSERT INTO bin_lots (client_id, sku, location_id, qty, received_at, lot_number, expiry_date)
                       VALUES (?,?,?,?,date('now'),'',NULL)`).run(clientId, sku, loc, take);
      db.prepare(`INSERT INTO stock_movements (sku, client_id, type, qty, reason, to_location, operator)
                  VALUES (?,?,?,?,?,?,?)`).run(sku, clientId, 'putaway', take, 'Located existing stock (no qty change)', loc, operator);
      room.set(sku, room.get(sku) - take);
      if (want > take) out.overflow.push({ sku, location: loc, wanted: want, placed: take });
      out.located++; out.units += take; out.skus.add(sku);
    }
  })();
  out.skus = [...out.skus];
  out.notInMaster = [...new Set(out.notInMaster)];
  out.alreadyLocated = [...new Set(out.alreadyLocated)];
  // Cells only — this function never changes a quantity, so it has no on-hand
  // figures to restore. The caller merges these into its own reversal record.
  out.snapshot = {
    cells: [...snapCells.entries()].map(([k, lots]) => {
      const i = k.indexOf('|');
      return { sku: k.slice(0, i), location: k.slice(i + 1), lots };
...
// Read-only dry run of setStockPositions — what WOULD each row do. This is
// what the confirm dialog shows, so the numbers someone approves are computed
// the same way the write computes them.
function previewStockPositions(clientId, rows, mode = 'add') {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  const out = { rows: 0, units: 0, skus: new Set(), newSkus: [], newBins: [], errors: [], short: [], cells: [] };
  const cellQty = db.prepare(`SELECT COALESCE(SUM(qty),0) AS n FROM bin_lots
    WHERE client_id=? AND sku=? AND location_id=?`);
  const findLoc = db.prepare('SELECT location_id FROM warehouse_locations WHERE location_id=?');
  for (const r of rows || []) {
    const sku = String(r.sku || '').trim();
    const loc = String(r.location || '').trim().toUpperCase();
    ...
    if (!sku || !Number.isFinite(qty) || qty < 0) {
      out.errors.push({ sku, location: loc, reason: 'missing sku or quantity' }); continue;
    }
    // A supersede COUNTS a row with no location — on-hand, unbinned — so the
    // preview must count it too, or the confirm promises a total the upload
    // will not reach. (Add/reduce still cannot apply one to a bin.)
    if (!loc) {
      if (mode !== 'set') {
        out.errors.push({ sku, location: '', reason: 'no location — cannot add to or reduce a bin' }); continue;
      }
      if (!get(sku, clientId) && !out.newSkus.includes(sku)) out.newSkus.push(sku);
      out.unbinnedRows = (out.unbinnedRows || 0) + 1;
      out.unbinnedUnits = (out.unbinnedUnits || 0) + qty;
      if (!out.unbinnedSkus) out.unbinnedSkus = [];
      if (out.unbinnedSkus.length < 30) out.unbinnedSkus.push({ sku, qty });
      out.rows++; out.units += qty; out.skus.add(sku);
      continue;
    }
    const item = get(sku, clientId);
    const binExists = !!findLoc.get(loc);
    if (mode === 'reduce') {
      if (!item)      { out.errors.push({ sku, location: loc, reason: 'SKU not in the item master — nothing to reduce' }); continue; }
      if (!binExists) { out.errors.push({ sku, location: loc, reason: 'bin does not exist — nothing to reduce' }); continue; }
    } else {
      if (!item && !out.newSkus.includes(sku)) out.newSkus.push(sku);
      if (!binExists && !out.newBins.includes(loc)) out.newBins.push(loc);
    }
    const before = binExists ? cellQty.get(clientId, sku, loc).n : 0;
    const after = mode === 'set' ? qty : mode === 'reduce' ? Math.max(0, before - qty) : before + qty;
    if (mode === 'reduce' && qty > before) out.short.push({ sku, location: loc, wanted: qty, had: before });
    if (out.cells.length < 200) out.cells.push({ sku, location: loc, before, after });
    out.rows++; out.units += qty; out.skus.add(sku);
  }
  out.skus = [...out.skus];
  // ── WHAT A SUPERSEDE WILL WIPE, SAID BEFORE IT HAPPENS ────────────────────
  // The whole point of the confirm is that the number someone approves is the
  // number they get. A supersede replaces the client's ENTIRE position, so the
  // preview has to name the stock that is NOT in the file and is therefore
  // about to go to zero — otherwise "740 units" reads as an addition and the
  // 104 that vanish are a surprise.
  if (mode === 'set') {
    // THE FILE IS THE POSITION: afterTotal is the sheet's own sum, always.
    // Every SKU with a usable quantity is counted — located or not — so there
    // is nothing left over to reason about and nothing "untouched".
    const inFile = new Set(out.skus);
    const rowsNow = db.prepare(
      `SELECT sku, IFNULL(stock_qty,0) AS n FROM inventory WHERE client_id=? AND IFNULL(stock_qty,0) > 0`
    ).all(clientId);
    out.currentTotal = rowsNow.reduce((n, r) => n + r.n, 0);
    out.afterTotal   = out.units;
    out.untouchedSkus = [];               // nothing is left standing any more
    out.untouchedUnits = 0;
    out.zeroSkus = rowsNow.filter(r => !inFile.has(r.sku)).map(r => ({ sku: r.sku, was: r.n }));
    out.zeroUnits = out.zeroSkus.reduce((n, r) => n + r.was, 0);
    // Stock on a SKU the file DOES list is replaced rather than zeroed —
    // counted here so the arithmetic on screen adds up.
    out.replacedUnits = out.currentTotal - out.zeroUnits;
  }
  return out;
}

// ── FINISH A SUPERSEDE THAT ONLY GOT HALF DONE ──────────────────────────────
// A supersede run before the whole-position fix cleared only the (sku, bin)
// CELLS its file named, leaving the rest of the client's stock standing. The
// file itself is gone, but the transaction's snapshot recorded exactly which
// cells it touched — so the position it INTENDED can be rebuilt from what is in
// those cells right now, with no re-upload.
//
// Returns rows in setStockPositions' own shape, so completing the job is just
// running the corrected `set` against them: one implementation, and the same
// preview, confirm, reporting and 3-day reversal as any other supersede.
function supersedeRowsFromSnapshot(clientId, snapshot) {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  const cellQty = db.prepare(`SELECT COALESCE(SUM(qty),0) AS n, MAX(IFNULL(lot_number,'')) AS lot
    FROM bin_lots WHERE client_id=? AND sku=? AND location_id=?`);
  const rows = [];
  const seen = new Set();
  for (const c of snapshot?.cells || []) {
    const k = `${c.sku}|${c.location}`;
    if (seen.has(k)) continue;
    seen.add(k);
    const r = cellQty.get(clientId, c.sku, c.location);
    rows.push({ sku: c.sku, location: c.location, qty: Number(r?.n) || 0, lot: r?.lot || '' });
  }
  return rows;
}

// ── AN UPLOAD MADE BEFORE THE TRANSACTION RECORD EXISTED ────────────────────
// The Inventory-tab uploader kept no snapshot until now, so an upload made
// before that cannot be reversed the ordinary way. It is not lost, though: each
// SKU it topped up wrote a `type='upload'` movement carrying THE DELTA, so the
// ledger itself says what to give back. Grouped by minute, because one upload
// writes its rows within a second or two of each other.
//
// HONEST LIMIT, and it is reported rather than hidden: a SKU the upload CREATED
// went in through upsert, which writes no movement row. The ledger cannot say
// how much of that SKU's stock came from the upload, so those are NAMED for a
// human instead of being guessed at.
function ledgerUploads(clientId, { days = 30 } = {}) {
  const db = _open();
  if (!db || !clientId) return [];
  const since = `-${Number(days) || 30} days`;
  // FILE UPLOADS — grouped by minute, because one upload writes its rows within
  // a second or two and has nothing else to key on.
  const ups = db.prepare(`SELECT substr(at,1,16) AS k, COUNT(*) AS skus, SUM(qty) AS units,
      MIN(at) AS from_at, MAX(at) AS to_at
    FROM stock_movements WHERE client_id=? AND type='upload' AND at >= datetime('now', ?)
    GROUP BY substr(at,1,16)`).all(clientId, since)
    .map(r => ({ kind: 'upload', key: r.k, label: `File upload · ${r.k}`, ...r }));
  // INBOUND RECEIPTS — keyed by the receipt itself (`Receipt IB-…`), which is
  // better than a timestamp: it names the job the stock came in on.
  const ins = db.prepare(`SELECT reason AS k, COUNT(*) AS skus, SUM(qty) AS units,
      MIN(at) AS from_at, MAX(at) AS to_at
    FROM stock_movements WHERE client_id=? AND type='inbound' AND at >= datetime('now', ?)
      AND IFNULL(reason,'') <> ''
    GROUP BY reason`).all(clientId, since)
    .map(r => ({ kind: 'inbound', key: r.k, label: `Inbound ${r.k}`, ...r }));
  // STOCK THAT WAS TAKEN OFF, so it can be put back. A take-off made before the
  // snapshot existed has no record of its own — but it wrote a compensating
  // NEGATIVE adjustment per SKU carrying the amount, which is all that is
  // needed. Matched on the reasons this system writes, never on negative
  // adjustments in general: a hand correction someone made on purpose is not
  // something to offer to reverse.
  const offs = db.prepare(`SELECT reason AS k, COUNT(*) AS skus, SUM(-qty) AS units,
      MIN(at) AS from_at, MAX(at) AS to_at
    FROM stock_movements WHERE client_id=? AND type='adjustment' AND qty < 0
      AND (reason LIKE 'stock upload of %undone' OR reason LIKE 'stock of %backed out%'
           OR reason LIKE 'stock posting undone%')
      AND at >= datetime('now', ?)
    GROUP BY reason`).all(clientId, since)
    .map(r => ({ kind: 'takeoff', key: r.k, label: `Stock taken off · ${r.k}`, ...r }));
  return [...ups, ...ins, ...offs]
    .sort((a, b) => String(b.to_at).localeCompare(String(a.to_at)))
    .map(r => ({ ...r, minute: r.kind === 'upload' ? r.key : undefined, id: `${clientId}|${r.kind}|${r.key}` }));
}

// Give back exactly what that posting added. Each SKU is undone with its own
// compensating movement, so the ledger keeps both halves of the story rather
// than pretending the stock was never booked.
//
// IT BACKS OUT A STOCK POSTING, IT DOES NOT DELETE ANYTHING ELSE. An inbound
// receipt really happened — the goods arrived and were counted — so the receipt
// record, its GRN and its trail are all left exactly as they are. What is being
// corrected is the stock having been booked twice, once by the receipt and once
// by a file upload; only one of them should stand.
function reverseLedgerUpload(clientId, spec, { operator = '', reason = 'stock posting undone' } = {}) {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  if (!clientId) throw new Error('clientId is required');
  const kind = typeof spec === 'string' ? 'upload' : String(spec?.kind || 'upload');
  const key  = typeof spec === 'string' ? spec : String(spec?.key || '');
  if (!key) throw new Error('which posting? (key is required)');
  if (!['upload', 'inbound', 'takeoff'].includes(kind)) throw new Error('kind must be upload, inbound or takeoff');
  // A TAKE-OFF IS PUT BACK, not taken off again. No sign flip is needed and one
  // would be wrong: its rows are ALREADY negative, so reading them as they
  // stand makes the same code add the stock back on. One implementation, and
  // the direction comes from the data rather than from a flag.
  const moves = kind === 'upload'
    ? db.prepare(`SELECT sku, SUM(qty) AS qty FROM stock_movements
        WHERE client_id=? AND type='upload' AND substr(at,1,16)=? GROUP BY sku`).all(clientId, key)
    : kind === 'inbound'
    ? db.prepare(`SELECT sku, SUM(qty) AS qty FROM stock_movements
        WHERE client_id=? AND type='inbound' AND reason=? GROUP BY sku`).all(clientId, key)
    : db.prepare(`SELECT sku, SUM(qty) AS qty FROM stock_movements
        WHERE client_id=? AND type='adjustment' AND qty < 0 AND reason=? GROUP BY sku`).all(clientId, key);
  if (!moves.length) throw new Error('No movements found for that posting');
  const out = { kind, key, skus: 0, units: 0, floored: [], created: [], snapshot: null };
  // BEFORE-SNAPSHOT, so the undo is itself undoable. An undo that cannot be
  // undone is a one-way door, and the floor walked through two of them in a row
  // and landed on zero.
  const before = [];
  db.transaction(() => {
    for (const m of moves) {
      const it = get(m.sku, clientId);
      if (!it) continue;
      before.push({ sku: m.sku, before_stock: Number(it.stock_qty) || 0 });
      const want = Number(m.qty) || 0;   // negative on a take-off = put it back
      const have = Number(it.stock_qty) || 0;
      if (want < 0) {
        // PUTTING STOCK BACK ON — nothing to floor against.
        const add = -want;
        db.prepare("UPDATE inventory SET stock_qty=?, updated_at=datetime('now') WHERE client_id=? AND sku=?")
          .run(have + add, clientId, m.sku);
        db.prepare('INSERT INTO stock_movements (sku,client_id,type,qty,reason,operator) VALUES (?,?,?,?,?,?)')
          .run(m.sku, clientId, 'adjustment', add, reason, operator);
        out.skus++; out.units += add;
        continue;
      }
      const take = Math.min(want, have);
      // Never below zero, and say so when the shelf no longer holds what was
      // booked — something has shipped since, and that is real.
      if (take < want) out.floored.push({ sku: m.sku, added: want, could_take: take });
      db.prepare("UPDATE inventory SET stock_qty=?, updated_at=datetime('now') WHERE client_id=? AND sku=?")
        .run(have - take, clientId, m.sku);
      db.prepare('INSERT INTO stock_movements (sku,client_id,type,qty,reason,operator) VALUES (?,?,?,?,?,?)')
        .run(m.sku, clientId, 'adjustment', -take, reason, operator);
      out.skus++; out.units += take;
    }
  })();
  // A file upload can also CREATE SKUs, which go in through upsert and write no
  // movement row — the ledger cannot say how much of those came from it, so
  // they are named for a human rather than guessed at. (An inbound receipt
  // always writes a movement, so this only applies to uploads.)
  if (kind === 'upload') {
    out.created = db.prepare(`SELECT sku, stock_qty FROM inventory
      WHERE client_id=? AND substr(IFNULL(first_added_at,''),1,16)=?
        AND sku NOT IN (SELECT sku FROM stock_movements WHERE client_id=? AND type='upload' AND substr(at,1,16)=?)`)
      .all(clientId, key, clientId, key);
  }
  out.snapshot = {
    at: new Date().toISOString(), cells: [], skus: before,
    // Taken AFTER the writes, so a later "has anything moved since" check does
    // not count this undo's own compensating rows.
    asOfMovementId: db.prepare('SELECT COALESCE(MAX(id),0) AS n FROM stock_movements').get().n,
  };
  return out;
}

// Where the movement ledger stands right now. A caller that writes stock
// outside setStockPositions still needs this to mark its own reversal window —
// it is what decides, later, whether anything has moved since.
function movementHighWater() {
  const db = _open();
  if (!db) return 0;
  return db.prepare('SELECT COALESCE(MAX(id),0) AS n FROM stock_movements').get().n;
}

// Which of these SKUs have physically moved since the ledger stood at
// sinceMovementId (reserve/release excluded — allocating moves no piece).
// This is the clean-reversal test: a SKU that shipped or was received since
// the import cannot have its position blindly restored.
function movedSkusSince(clientId, skus, sinceMovementId) {
  const db = _open();
  if (!db || !clientId || !skus?.length) return [];
  const moved = new Set();
  for (let i = 0; i < skus.length; i += 400) {
    const chunk = skus.slice(i, i + 400);
    const q = db.prepare(`SELECT DISTINCT sku FROM stock_movements
      WHERE client_id=? AND id > ? AND type NOT IN ('reserve','release')
        AND sku IN (${chunk.map(() => '?').join(',')})`).all(clientId, Number(sinceMovementId) || 0, ...chunk);
    for (const r of q) moved.add(r.sku);
  }
  return [...moved];
}

// Put the snapshotted position back — the reversal half of the 3-day window.
// Restores each touched (sku, bin) cell's exact lot rows and each SKU's
// on-hand figure as they were BEFORE the import. `excludeSkus` carries the
// SKUs whose stock moved since (the caller decides whether to skip them);
// a SKU the import CREATED (before_stock === null) is deleted again only if
// it still has no stock anywhere else.
function reverseStockPositions(clientId, snapshot, { operator = '', reason = 'mass stock upload reversed', excludeSkus = [] } = {}) {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  if (!clientId || !snapshot?.cells) throw new Error('nothing to reverse');
  const skip = new Set(excludeSkus.map(s => String(s)));
  const out = { cells: 0, skus: 0, skippedSkus: [...skip] };
  db.transaction(() => {
    for (const c of snapshot.cells) {
      if (skip.has(c.sku)) continue;
      db.prepare('DELETE FROM bin_lots WHERE client_id=? AND sku=? AND location_id=?').run(clientId, c.sku, c.location);
      for (const l of c.lots || []) {
        ...
        db.prepare(`INSERT INTO bin_lots (client_id, sku, location_id, qty, received_at, lot_number, expiry_date)
                    VALUES (?,?,?,?,?,?,?)`)
          .run(clientId, c.sku, c.location, Number(l.qty), l.received_at || null, l.lot_number || '', l.expiry_date || null);
      }
      out.cells++;
    }
    for (const s of snapshot.skus || []) {
      if (skip.has(s.sku)) continue;
      if (s.before_stock === null || s.before_stock === undefined) {
        // The import created this SKU. If nothing else gave it stock, remove
        // the row again; if something did, keep it and just restore the count.
        const n = db.prepare('SELECT COALESCE(SUM(qty),0) AS n FROM bin_lots WHERE client_id=? AND sku=?').get(clientId, s.sku).n;
        if (n <= 0) { db.prepare('DELETE FROM inventory WHERE client_id=? AND sku=?').run(clientId, s.sku); out.skus++; continue; }
        db.prepare('UPDATE inventory SET stock_qty=? WHERE client_id=? AND sku=?').run(n, clientId, s.sku);
      } else {
        db.prepare('UPDATE inventory SET stock_qty=? WHERE client_id=? AND sku=?').run(Number(s.before_stock) || 0, clientId, s.sku);
      }
      db.prepare(`INSERT INTO stock_movements (sku, client_id, type, qty, reason, operator)
                  VALUES (?,?,?,?,?,?)`)
        .run(s.sku, clientId, 'adjustment', Number(s.before_stock) || 0, reason, operator);
      out.skus++;
    }
  })();
  return out;
...
function outboundMovements(clientId, { from, to, limit = 20000 } = {}) {
  ...
  // ADJUSTMENTS BELONG IN THE CLIENT'S RECORD TOO. Reported from the floor: two
  // units were written off as damaged during a receipt, the balance was
  // corrected, and the client had no way to see it happen — this list read only
  // reserve/release/outbound, so a write-off left no trace on their side. It is
  // their stock; a change to it is their business, and the reason we typed is
  // what explains it. `quarantine_release` rides along for the same reason: held
  // stock becoming sellable again is a real event on their account.
  const where = ['sm.client_id = ?',
    "sm.type IN ('reserve','release','outbound','adjustment','quarantine_release')"];
...
// Every physical movement of a client's goods in a period, plus the balance
// they started and ended with. `reserve`/`release` are DELIBERATELY EXCLUDED:
// allocating stock to an order and giving it back moves no piece off a shelf,
// and a statement that lists them reads as churn the client cannot reconcile
// against anything they can see. What remains is what actually happened —
// received, shipped, adjusted, transferred, written off.
// ── WHICH MOVEMENTS A CLIENT SEES ───────────────────────────────────────────
// EVERY movement type is classified here, deliberately, in one place.
//
//   'account'  — the client's goods entered or left THEIR ACCOUNT. Real to them.
//   'internal' — the piece moved between places INSIDE the warehouse (bin →
//                bench, bin → bin), or was earmarked. Nothing entered or left.
//
// WHY THIS EXISTS AS A MAP AND NOT A FILTER. It used to be a deny-list
// ("everything except reserve and release"), which meant any movement type
// added later was included in the client's statement BY DEFAULT. Bin-level
// picking was added afterwards, its 'pick' rows joined the statement unasked,
// and every unit was then counted twice — once leaving its bin, once leaving
// the account. Reported live (Mayer2026, 2026-08-14): three parcels read as
// "Shipped 6", with the derived opening inflated to match. Nobody made a
// mistake; the default did.
//
// An allow-list alone would fail the opposite way — a new CLIENT-VISIBLE type
// (a write-off, say) would be silently missing from their statement, which is
// worse. So: unknown types still COUNT (nothing real is ever hidden), and
// `unclassifiedMovementTypes()` reports them so they get classified
// deliberately instead of drifting in unnoticed. See the health check.
const MOVEMENT_KINDS = {
  outbound:   'account',    // shipped to the customer
  inbound:    'account',    // received into stock
  adjustment: 'account',    // a correction to the count
  reserve:    'internal',   // earmarked for an order; nothing moved
  release:    'internal',   // un-earmarked; nothing moved
  pick:       'internal',   // bin → packing bench
  transfer:   'internal',   // bin → bin
  // Both surfaced by unclassifiedMovementTypes() doing its job — they were
  // counting in client statements with nothing saying what they were.
  upload:            'account',   // stock put on the books by a file upload
  quarantine_release: 'account',  // held stock released back into sellable
};
const _INTERNAL_TYPES = Object.entries(MOVEMENT_KINDS)
  .filter(([, kind]) => kind === 'internal').map(([t]) => t);
const _STATEMENT_TYPES =
  `sm.type NOT IN (${_INTERNAL_TYPES.map(t => `'${t}'`).join(',')})`;

// Movement types present in the ledger that nobody has classified. Empty is
// the healthy state; anything here is counting as client-visible by default
// and needs a deliberate decision.
function unclassifiedMovementTypes() {
  const db = _open();
  if (!db) return [];
  try {
    return db.prepare('SELECT DISTINCT type FROM stock_movements').all()
      .map(r => String(r.type || '').trim())
      .filter(t => t && !MOVEMENT_KINDS[t]);
  } catch (_) { return []; }
}

function transactionMovements(clientId, { from, to, limit = 50000 } = {}) {
  const db = _open();
  if (!db || !clientId) return [];
  const where = ['sm.client_id = ?', _STATEMENT_TYPES];
  const params = [clientId];
  if (from) { where.push('date(sm.at) >= date(?)'); params.push(from); }
  if (to)   { where.push('date(sm.at) <= date(?)'); params.push(to); }
  return db.prepare(`SELECT sm.at, sm.type, sm.sku, sm.qty, sm.reason, sm.order_id, sm.operator,
      sm.from_location, sm.to_location, i.name AS product, i.barcode AS barcode
    FROM stock_movements sm
    LEFT JOIN inventory i ON i.client_id = sm.client_id AND i.sku = sm.sku
    WHERE ${where.join(' AND ')}
    ORDER BY sm.at ASC, sm.id ASC LIMIT ?`).all(...params, Number(limit) || 50000);
}

// The net movement per SKU BEFORE the period — which, worked back from what is
// on the shelf now, is how the opening balance is derived. Storing a nightly
// snapshot would be faster but would drift the moment anyone backdates a
// correction; deriving it keeps the statement reconcilable to the live count.
function movementNetBefore(clientId, from) {
  const db = _open();
  if (!db || !clientId || !from) return new Map();
  const rows = db.prepare(`SELECT sm.sku,
      SUM(sm.qty) AS net
    FROM stock_movements sm
    WHERE sm.client_id = ? AND ${_STATEMENT_TYPES} AND date(sm.at) < date(?)
    GROUP BY sm.sku`).all(clientId, from);
  return new Map(rows.map(r => [String(r.sku), Number(r.net) || 0]));
}

// Net movement WITHIN the period, per SKU — so closing = opening + in - out
// reconciles on the face of the report.
function movementNetIn(clientId, { from, to } = {}) {
  const db = _open();
  if (!db || !clientId) return new Map();
  const where = ['sm.client_id = ?', _STATEMENT_TYPES];
  const params = [clientId];
  if (from) { where.push('date(sm.at) >= date(?)'); params.push(from); }
  if (to)   { where.push('date(sm.at) <= date(?)'); params.push(to); }
  const rows = db.prepare(`SELECT sm.sku,
      -- THE LEDGER STORES SIGNED QUANTITIES: an outbound row is already
      -- negative. Splitting on the TYPE name double-counted that sign and
      -- produced "Shipped -3" with a balance that would not reconcile.
      -- Splitting on the sign is both correct and future-proof against a new
      -- movement type nobody remembers to add here.
      SUM(CASE WHEN sm.qty < 0 THEN -sm.qty ELSE 0 END) AS out_qty,
      SUM(CASE WHEN sm.qty > 0 THEN  sm.qty ELSE 0 END) AS in_qty
    FROM stock_movements sm WHERE ${where.join(' AND ')} GROUP BY sm.sku`).all(...params);
  return new Map(rows.map(r => [String(r.sku), { in: Number(r.in_qty) || 0, out: Number(r.out_qty) || 0 }]));
}
...
// `claimed` is an OPTIONAL Map(lot_id → units already promised by earlier calls
// in the SAME planning run — see newPickClaim(). Pass one whenever more than
// one call plans against the same stock:
//
//   THIS FUNCTION IS STATELESS. It reads bin_lots and decrements nothing, so
//   every call starts again from the full pool. A caller that calls it twice
//   for one SKU — a wave row per printed location, or a line per order in a
//   batch — promises the SAME PHYSICAL UNITS TWICE. Reported from the floor:
//   a bin holding 24 was asked for 28 across two wave rows while a second bin
//   holding 6 was never visited. The ledger is what stops that.
// A ledger for ONE planning run: which lots have already been promised, and how
// many units. Hand the same object to every allocatePick call planning the same
// run, so no two rows can be sent to the same physical units.
// Every client that actually holds a catalogue. A client can exist ONLY as an
// item master — that is what onboarding creates first — so anything deriving
// client identity from batches or profiles alone will not see them.
function listClientIds() {
  const db = _open();
  if (!db) return [];
  try {
    return db.prepare('SELECT DISTINCT client_id AS c FROM inventory WHERE client_id IS NOT NULL AND client_id <> \'\'')
      .all().map(r => r.c).filter(Boolean);
  } catch (_) { return []; }
}

function newPickClaim() { return new Map(); }

function allocatePick(clientId, sku, qty, strategy = 'fefo', unitsPerCarton = 1, claimed = null) {
  ...
  // Start from what is left AFTER everything already promised this run.
  const rem = new Map(lots.map(l => [l.id, Math.max(0, l.qty - (claimed ? (claimed.get(l.id) || 0) : 0))]));
  const picks = [];
  const takeFrom = (subset, amount) => {
    for (const l of subset) {
      ...
      if (claimed) claimed.set(l.id, (claimed.get(l.id) || 0) + t);
...
module.exports = {
  ...
  transactionMovements, movementNetBefore, movementNetIn,
  binLotsForSku, suggestLocationForSku, suggestSlotForNewSku, suggestPutaway, parseLocationId, skuVelocity, allocatePick, newPickClaim, listClientIds, consumeAllocations, consumeFromLocation, restoreLots, zeroBinLot, caseBreakReplenish, binnedQty, resolveBinnedSku, resolveBinnedFor, skusForBarcode, binLocationsBySku, locateExistingStock, enforceBinCap, reconcileBinOverage, stagingQty,
  // Serial numbers
  ...
  previewStockPositions, movedSkusSince, reverseStockPositions, supersedeRowsFromSnapshot,
  movementHighWater, ledgerUploads, reverseLedgerUpload,
  MOVEMENT_KINDS, unclassifiedMovementTypes,
