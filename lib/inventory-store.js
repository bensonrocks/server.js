...
function updateLocation(locationId, { length_cm, width_cm, height_cm, capacity, environment, active } = {}) {
  const db = _open();
  if (!db) throw new Error('inventory unavailable');
  ...
  db.prepare(`UPDATE warehouse_locations SET length_cm=?, width_cm=?, height_cm=?, capacity=?, environment=?, active=? WHERE location_id=?`)
    .run(v(length_cm, loc.length_cm), v(width_cm, loc.width_cm), v(height_cm, loc.height_cm),
      v(capacity, loc.capacity), environment || loc.environment, active === undefined ? loc.active : (active ? 1 : 0), locationId);
  return db.prepare('SELECT * FROM warehouse_locations WHERE location_id=?').get(locationId);
}
...
module.exports = {
  ...
  createLocation, getLocations, getLocation, updateLocation, locationOccupancy, binCapacityCheck, binContents, stockByLocation,
  transferStock, placeStock, locationStockForClient,
  binLotsForSku, suggestLocationForSku, allocatePick, consumeAllocations, restoreLots, binnedQty, stagingQty,
  // Suppliers
