...
function getReorderSuggestions(clientId) {
  ...
  return db.prepare(`SELECT i.sku, i.name, i.available_qty, i.reorder_point, i.reorder_point - i.available_qty as needed,
    ...
    FROM inventory i LEFT JOIN suppliers s ON s.supplier_id=ssm.supplier_id AND s.client_id=i.client_id
    WHERE i.client_id=? AND i.available_qty <= i.reorder_point AND i.available_qty > 0
