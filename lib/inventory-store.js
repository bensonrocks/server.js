...
// 'pick' and 'transfer' are excluded for the SAME reason: they move a piece
// BETWEEN PLACES INSIDE the warehouse (bin → bench, bin → bin), not out of the
// client's account. Only the account-level 'outbound' is a shipment.
// REPORTED LIVE (Mayer2026, 2026-08-14): three parcels read as "Shipped 6"
// because every unit appeared twice — once as the bin pick, once as the
// shipment — and the derived opening balance was inflated to match. The stock
// itself was always right (the pick decrements bin_lots, the outbound
// decrements stock_qty); only the statement double-counted.
const _STATEMENT_TYPES = "sm.type NOT IN ('reserve','release','pick','transfer')";
