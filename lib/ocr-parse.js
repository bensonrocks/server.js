...
// Matches formats like: BC-003-035, BD-002-022, A01-02-03, AC-007-003-B
const LOCATION_CODE_PAT = /^[A-Z]{1,3}-\d{2,4}-\d{2,4}[A-Z]?$/i;
    ...
    for (let i = 0; i < lines.length - 1; i++) {
      for (const { re, priority } of TWO_LINE_ORDER_KEYS) {
        if (re.test(lines[i].trim())) {
          const val = lines[i + 1].trim().replace(/[^A-Z0-9\-\/]/gi, '').toUpperCase();
