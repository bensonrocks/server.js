...
// ── A CARRIER TRACKING NUMBER WHERE A NAME SHOULD BE ──────────────────────────
// Keyfields prints the Betime picking list with the TracX waybill in the
// "Consignee" box and the buyer's name in "Consignee Address" — the two are
// swapped. Reported from the floor as "tracx labels always having issues": the
// waybill was being filed as the customer's NAME, the order carried no waybill
// at all, and a TracX label (which prints exactly that waybill) had nothing on
// the order to match. One shape test, shared by every parser that reads a
// consignee, so the three of them cannot disagree about what a waybill looks
// like. The prefixes mirror lib/label-extract.js, which reads the same numbers
// off the label; the generic tail is the same shape it accepts.
const TRACKING_SHAPE = /^(?:TXSGD\d{8,}|SGDEX\d{8,}|SPXSG\d{8,}|LZSGD\d{8,}|[A-Z]{2,6}\d{9,18})$/i;
function looksLikeTrackingNumber(v) {
  const s = String(v || '').replace(/[\s\-]/g, '').trim();
  return !!s && TRACKING_SHAPE.test(s);
}
...
// Matches: BC-003-035, AC-007-003-B, AB-005001-A, AB-006-001-B, DMG-2, DMMG-2, BIN-1
// Prefix allows up to 4 letters because OCR sometimes doubles a letter (DMG→DMMG).
// Pattern: 1-4 letter prefix, 1-3 hyphen-separated digit groups (1-6 digits each),
// optional hyphen + 1-2 letter suffix.
const LOCATION_CODE_PAT = /^[A-Z]{1,4}(-\d{1,6}){1,3}(-[A-Z]{1,2})?$/i;

// Unit-of-measure keywords that immediately follow the quantity.
// CART[A-Z0-9]{0,3} catches OCR variants of CARTO: CARTC, CART0, CARTOS, CARTON, etc.
const UOM_RE = /^(?:EACH|EA|PCS|PIECES|BOX|CTN|CART[A-Z0-9]{0,3}|UOM)$/i;
...
const OCR_TRAIL_MAP = { '2': 'Z', '7': 'Z' };

function fixOcrConfusions(code) {
  // All-digit codes: leading letter fix takes priority, then trailing
  if (/^\d{7,}$/.test(code)) {
  ...
  // OCR doubling of terminal Z: Z misread as 2 or 7 immediately before genuine Z
  // e.g. 010720267Z (OCR sees Z as both 7 and Z) → 01072026Z
  const dupZ = code.match(/^(\d{6,})[27]Z$/i);
  if (dupZ) return dupZ[1] + 'Z';
  return code;
}
...
function parseOcrPicklist(text) {
  ...
  // THE CONSIGNEE BOX MAY HOLD THE WAYBILL (see looksLikeTrackingNumber). Read
  // it first, on its own: a tracking number is the order's waybill, never its
  // customer, and must not be handed to the name patterns below.
  let waybillNumber = '';
  const rawConsignee = extractKV(lines, /consignee\s*[:\s]+(.*)/i) || '';
  const consigneeTok = rawConsignee.split(/\s{2,}/)[0].trim();
  if (looksLikeTrackingNumber(consigneeTok)) waybillNumber = consigneeTok.replace(/[\s\-]/g, '').toUpperCase();
  ...
  for (const re of CUSTOMER_PATTERNS) {
    ...
    if (val) {
      ...
      // A waybill is not a name — try the next pattern.
      if (looksLikeTrackingNumber(customerName)) { customerName = ''; continue; }
      if (customerName.length >= 2) break;
      customerName = '';
    }
  }
  // When the waybill sat in the Consignee box, the buyer's name is usually in
  // "Consignee Address" — taken only when it reads as a NAME (short, no
  // digits), never when it is a real street address.
  if (!customerName && waybillNumber) {
    const addr = (extractKV(lines, /consignee\s*address\s*[:\s]+(.*)/i) || '').split(/\s{2,}/)[0].trim();
    if (addr && addr.length <= 40 && !/\d/.test(addr) && /[A-Za-z]{2}/.test(addr)) customerName = addr;
  }
  ...
  // A marketplace order id in the Reference box (13–18 digits — the shape a
  // Lazada/Shopee id has and the shape a carrier label prints as "Order No")
  // rides on the order as po_number, where the label matcher can find it.
  const poNumber   = /^\d{13,18}$/.test(reference) ? reference : '';
  ...
  for (let i = dataStart; i < lines.length; i++) {
    ...
    // ── Skip bin/location code (e.g. BC-003-035, DMMG-2) ────────────────────
    let locationSkipped = false;
    let lineLocation = '';
    if (cursor < qtyIdx && LOCATION_CODE_PAT.test(tokens[cursor])) {
      lineLocation = tokens[cursor].toUpperCase();
      cursor++;
      locationSkipped = true;
    }

    // ── SKU: first valid product-code token ──────────────────────────────────
    // When a location code was just skipped the immediately following token is
    // the SKU column — accept 3+ chars (not full MIN_SKU_LEN) so that a single
    // OCR-dropped digit (5603→603) does not silently lose the whole row.
    const skuMinLen = locationSkipped ? 3 : MIN_SKU_LEN;
    let sku = '';
    const descTokens = [];
    for (let t = cursor; t < qtyIdx; t++) {
      const tok = tokens[t];
      if (!sku) {
        if (tok.length < skuMinLen || tok.length > 40) continue;
        if (SKIP_SKU.has(tok.toLowerCase())) continue;
        if (/^0+$/.test(tok)) continue;                 // all-zero tokens are OCR noise
        if (EXPIRY_DATE_PAT.test(tok)) continue;        // expiry dates are not SKUs
        ...
        if (/^[A-Z0-9][A-Z0-9_\-]{2,}$/i.test(tok)) {
          ...
          // OCR inserts a phantom B between an alpha prefix and digit body:
          // e.g. Tesseract reads "PH6930" as "PHB6930". Strip it.
          s = s.replace(/^([A-Z]{1,4})B(\d{3,}[A-Z]{0,2})$/, '$1$2');
          sku = s;
          continue;
        }
      } else {
...
module.exports = { parseOcrPicklist, looksLikeTrackingNumber };
