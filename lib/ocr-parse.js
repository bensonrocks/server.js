...
// Matches: BC-003-035, AC-007-003-B, AB-005001-A, AB-006-001-B (1-3 letter prefix,
// 1-3 hyphen-digit-groups of 2-6 digits, optional hyphen+1-2 letter suffix)
const LOCATION_CODE_PAT = /^[A-Z]{1,3}(-\d{2,6}){1,3}(-[A-Z]{1,2})?$/i;

// ── OCR character-confusion corrections ───────────────────────────────────────
// Common OCR misreads: certain digits are printed letters on most WMS documents.
// Only applied when the full extracted code is all-numeric AND long (7+ chars),
// which strongly suggests a leading letter was misread as a digit.
const OCR_DIGIT_TO_LETTER = { '0': 'O', '5': 'S', '8': 'B', '6': 'G', '1': 'I' };

function fixOcrLeadingChar(code) {
  if (/^\d{7,}$/.test(code)) {
    const letter = OCR_DIGIT_TO_LETTER[code[0]];
    if (letter) return letter + code.slice(1);
  }
  return code;
}
...
function parseOcrPicklist(text) {
  ...
  for (const re of ORDER_PATTERNS) {
    for (const line of lines) {
      ...
      if (m && m[1]) {
        ...
        if (cand.length >= 2) { orderNumber = fixOcrLeadingChar(cand); break; }
      }
    }
  ...
  const issueNo    = rawIssue .replace(/[^A-Z0-9\-]/gi, '').toUpperCase();
  const pickTicket = rawTicket.replace(/[^A-Z0-9\-]/gi, '').toUpperCase();
  const reference  = rawRef   .replace(/[^A-Z0-9\-]/gi, '').toUpperCase();
  ...
  // Minimum SKU length: stricter for Picking List to block background-text noise
  const MIN_SKU_LEN = isPickingList ? 6 : 3;
  ...
  for (let i = dataStart; i < lines.length; i++) {
    ...
    // ── Qty: rightmost plain integer within a sensible range ─────────────────
    ...
    // ── Lot/batch: token just after qty ──────────────────────────────────────
    const lotTok = tokens[qtyIdx + 1] || '';
    const batchNumber = /^[A-Z]\d{3,}$/i.test(lotTok) ? lotTok.toUpperCase() : '';
