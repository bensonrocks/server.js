...
// HOW LONG ONE LABEL MAY HOLD THE BROWSER. These were 45s + 25s, and an
// attempt makes two round trips (open, then sign in and open again), so one
// label that ZORT had not generated yet could hold Chromium — in the SAME
// container as the app — for well over two minutes, and the outbox tried
// them one after another. Reported as "users are experiencing a lag". A print
// page that has a label serves it within seconds; a page that does not will
// not serve it because we waited longer.
const ZORT_WEB_NAV_TIMEOUT = Number(process.env.ZORT_WEB_NAV_TIMEOUT || 20000);
const ZORT_WEB_PDF_WAIT_MS = Number(process.env.ZORT_WEB_PDF_WAIT_MS || 12000);
...
//
// "PLAYWRIGHT IS INSTALLED" IS NOT "CHROMIUM CAN LAUNCH". Reported from the
// floor as "zort has labels, why can't I pull?": the store form read the
// worker as available because `require('playwright')` succeeded, while every
// actual launch was failing (the postinstall's browser download or its shared
// libraries missing on the deployment) — and that failure went into a field
// nobody's screen showed. So a launch failure is REMEMBERED here and reported
// by `available()` for ZORT_WEB_LAUNCH_RETRY_MS, after which the next attempt
// re-tests it (a redeploy that fixes the browser must not need a restart to be
// noticed). `probeLaunch()` tests it deliberately — at boot, and from the
// health check — so the form and the tap say the truth BEFORE a packer finds
// out at the bench.
const ZORT_WEB_LAUNCH_RETRY_MS = Number(process.env.ZORT_WEB_LAUNCH_RETRY_MS || 30 * 60 * 1000);
let _launchFail = '', _launchFailAt = 0, _launchOkAt = 0;
function available() {
  ...
  if (_launchFail && Date.now() - _launchFailAt < ZORT_WEB_LAUNCH_RETRY_MS) {
    return { ok: false, launchFailed: true, why: `Chromium could not be launched on this server (${_launchFail}) — the browser download or its system libraries are missing on the deployment. On Railway, keep nixpacks.toml (it installs Chromium via nix) or set ZORT_BROWSER_PATH to an installed Chromium. Re-tested automatically every ${Math.round(ZORT_WEB_LAUNCH_RETRY_MS / 60000)} min.` };
  }
  return { ok: true, launchTestedAt: _launchOkAt ? new Date(_launchOkAt).toISOString() : null, executable: _execUsed || null };
}
...
async function _getBrowser() {
  ...
  _launching = (async () => {
    const opts = {
      headless: true,
      // Containers have no sandbox user namespace and a tiny /dev/shm. The
      // rest keeps Chromium as small as a print viewer needs: no extensions,
      // no background fetches, no sync, no audio, and its renderers capped —
      // it shares the container's CPU and memory with the app itself.
      args: ['--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu',
             '--disable-extensions', '--no-first-run', '--disable-background-networking',
             '--disable-sync', '--disable-default-apps', '--mute-audio',
             '--renderer-process-limit=2', '--disable-features=Translate,BackForwardCache'],
    };
    // WHICH CHROMIUM. ZORT_BROWSER_PATH, when set, is the only one tried — an
    // operator who named a binary wants that binary or an error about it.
    // Otherwise Playwright's own download first (the postinstall), then any
    // Chromium the deployment has on its PATH (`nixpacks.toml` adds one via
    // nix on Railway, where the download's shared libraries are missing) —
    // so nobody has to know a nix store path to set a variable to.
    const candidates = process.env.ZORT_BROWSER_PATH
      ? [{ label: 'ZORT_BROWSER_PATH', executablePath: process.env.ZORT_BROWSER_PATH }]
      : [{ label: 'playwright download' }, ..._systemChromiums().map(p => ({ label: p, executablePath: p }))];
    const errs = [];
    for (const c of candidates) {
      try {
        _browser = await pw.chromium.launch(c.executablePath ? { ...opts, executablePath: c.executablePath } : opts);
        _execUsed = c.label;
        _launchFail = ''; _launchFailAt = 0; _launchOkAt = Date.now();
        return _browser;
      } catch (e) {
        // Keep the first line only — Playwright's launch error is a paragraph
        // of install instructions, and the one line that names the missing
        // binary or library is what an operator needs.
        errs.push(`${c.label}: ${String(e.message || e).split('\n').map(s => s.trim()).filter(Boolean)[0].slice(0, 160)}`);
      }
    }
    _launchFail = errs.join(' · ').slice(0, 400);
    _launchFailAt = Date.now();
    throw new Error(`Chromium could not be launched — ${_launchFail}`);
  })();
  try { return await _launching; } finally { _launching = null; }
}
let _execUsed = '';
// Every Chromium this machine offers on its PATH or in the usual places —
// found, never assumed. `command -v` is what a shell does; the fixed paths
// cover a distro package with nothing on PATH for a service user.
function _systemChromiums() {
  const out = [];
  const seen = new Set();
  const add = p => { p = String(p || '').trim(); if (p && !seen.has(p) && fs.existsSync(p)) { seen.add(p); out.push(p); } };
  for (const name of ['chromium', 'chromium-browser', 'google-chrome', 'google-chrome-stable', 'chrome']) {
    try { add(require('child_process').execFileSync('/bin/sh', ['-c', `command -v ${name}`], { encoding: 'utf8', timeout: 3000 }).trim()); } catch (_) {}
  }
  for (const p of ['/usr/bin/chromium', '/usr/bin/chromium-browser', '/usr/bin/google-chrome', '/usr/bin/google-chrome-stable',
                   '/usr/lib/chromium/chromium', '/snap/bin/chromium', '/opt/google/chrome/chrome']) add(p);
  return out;
}

// Deliberately test that the browser really launches. Cheap (one launch, then
// the idle timer closes it) and the only way to know before a packer does.
async function probeLaunch() {
  const avail = available();
  if (!avail.ok && !avail.launchFailed) return avail;
  try { await _getBrowser(); _touchIdle(); return { ok: true, launchTestedAt: new Date(_launchOkAt).toISOString(), executable: _execUsed }; }
  catch (e) { return { ok: false, launchFailed: true, why: available().why || String(e.message || e) }; }
}
function _touchIdle() {
...
async function fetchLabelPdfViaBrowser(store, pageUrl, { _retried } = {}) {
  ...
  try {
    ...
    await context.route('**/*', async (route) => {
      ...
      // Pictures, fonts and media are never the label and cost the container
      // bandwidth and render time it is short of — dropped outright.
      if (!looksPdf && /^(image|font|media)$/.test(req.resourceType())) { return route.abort().catch(() => {}); }
...
// READ THE PAGE'S OWN FUNCTION, not just its buttons. The floor supplied the
// real entry point — `printMainMarketplaceDocument("shippingLabelPDF")` on
// /Sell/list — so the click is a `page.evaluate` away. What is still unknown is
// what that call DOES: whether it posts somewhere, opens a print window, or
// queues a Task Manager job, and whether it acts on the grid's selected rows.
// Its own source answers all three at once, so the capture reports it verbatim
// rather than making me guess at the behaviour of a function I can see the name
// of. Names are matched loosely because the sibling calls (print*Document) are
// the ones that say what the family does.
const _CTRL_PAT = /print|label|awb|waybill|shipping|ship|marketplace|task/i;
const _FN_PAT = /^(print|export|download|get|do)?.*(document|label|shipping|marketplace)/i;
async function capturePage(store, pageUrl, { _retried, fns = [] } = {}) {
  const avail = available();
  if (!avail.ok) throw new Error(`browser worker unavailable — ${avail.why}`);
  if (!store.webEmail || !store.webPassword) throw new Error('no ZORT web login on the store — add it on the store form');
  if (!/^https?:\/\//i.test(String(pageUrl))) throw new Error('give the full https URL of the ZORT page you press Print on');
  // THE URL IS PINNED TO ZORT'S OWN HOST. Without this the route drives a
  // headless browser, inside the deployment's network, to any address a caller
  // names — the ordinary server-side request forgery shape, which would reach
  // localhost and the cloud metadata endpoint as easily as a website. It is
  // admin-or-master gated, so it is not an open door; it is closed anyway
  // because the cost of doing it properly is nil, the same reasoning the ZORT
  // push receiver's own hardening records.
  //
  // It is also simply what the tool IS: it captures a ZORT screen. There is no
  // legitimate reason to point it anywhere else, so the restriction costs no
  // capability at all.
  {
    let want, got;
    try { want = new URL(ZORT_WEB_BASE).host; got = new URL(String(pageUrl)).host; }
    catch { throw new Error('that does not parse as a URL — copy the whole address out of the browser bar'); }
    if (got.toLowerCase() !== want.toLowerCase()) {
      throw new Error(`this only captures pages on ${want} — the address given was ${got}`);
    }
  }
  ...
  try {
    ...
    // The page's own functions, by source. This is what turns "there is a
    // button called Print" into "here is exactly what pressing it does".
    const functions = await page.evaluate(({ wanted, patSrc }) => {
      const pat = new RegExp(patSrc, 'i');
      const out = [];
      const seen = new Set();
      const take = (name) => {
        if (seen.has(name) || out.length >= 12) return;
        let fn; try { fn = window[name]; } catch { return; }
        if (typeof fn !== 'function') return;
        seen.add(name);
        let src = ''; try { src = String(fn); } catch { src = '(source unavailable)'; }
        out.push({ name, source: src.slice(0, 4000), truncated: src.length > 4000 });
      };
      for (const n of wanted) take(n);                 // the ones we were told about
      for (const n of Object.getOwnPropertyNames(window)) {  // and their siblings
        if (out.length >= 12) break;
        if (pat.test(n)) take(n);
      }
      return out;
    }, { wanted: fns, patSrc: _FN_PAT.source }).catch(() => []);

    // How rows are chosen. `printMainMarketplaceDocument` reads like it acts on
    // the grid's SELECTION, so the automation has to know how to make one.
    const selection = await page.evaluate(() => {
      const boxes = Array.from(document.querySelectorAll('input[type="checkbox"]')).slice(0, 8);
      return boxes.map(b => ({
        id: b.id || '', name: b.name || '',
        cls: (typeof b.className === 'string' ? b.className : '').slice(0, 60),
        value: (b.value || '').slice(0, 40),
        checked: !!b.checked,
      }));
    }).catch(() => []);
    ...
    return {
      ...
      bytes: html.length, htmlPath, shotPath, controls, functions, selection,
...
module.exports = { available, probeLaunch, fetchLabelPdfViaBrowser, capturePage, forgetSession, ZORT_WEB_BASE };
