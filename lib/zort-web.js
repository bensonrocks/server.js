...
function available() {
  ...
  if (_launchFail && Date.now() - _launchFailAt < ZORT_WEB_LAUNCH_RETRY_MS) {
    return { ok: false, launchFailed: true, why: `Chromium could not be launched on this server (${_launchFail}) — the browser download or its system libraries are missing on the deployment; set ZORT_BROWSER_PATH to an installed Chromium, or install it. Re-tested automatically every ${Math.round(ZORT_WEB_LAUNCH_RETRY_MS / 60000)} min.` };
  }
  return { ok: true, launchTestedAt: _launchOkAt ? new Date(_launchOkAt).toISOString() : null };
}
...
async function _getBrowser() {
  ...
  _launching = (async () => {
    ...
    try {
      ...
      _launchFail = String(e.message || e).split('\n').map(s => s.trim()).filter(Boolean)[0].slice(0, 220);
...
async function probeLaunch() {
  ...
  try { await _getBrowser(); _touchIdle(); return { ok: true, launchTestedAt: new Date(_launchOkAt).toISOString() }; }
