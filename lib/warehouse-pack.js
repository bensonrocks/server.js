...
module.exports = function createPacking(db, ordersApi) {
  ...
  function listPackOrders({ status, waveId } = {}) {
    ...
    return db.prepare(sql).all(...params).map(po => {
      const order = ordersApi.getOrder(po.order_id);
      return { ...po, clientName: order ? order.clientName : '', channel: order ? order.channel : '' };
    });
  }
  ...
  function listShipments({ status } = {}) {
    ...
    if (status) { sql += ' AND status = ?'; params.push(status); }
    sql += ' ORDER BY created_at DESC';
    return db.prepare(sql).all(...params).map(s => {
      const order = ordersApi.getOrder(s.order_id);
      return { ...s, clientName: order ? order.clientName : '' };
    });
  }
