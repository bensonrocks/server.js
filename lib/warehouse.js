...
module.exports = function createWarehouse(db) {
  ...
  function addItem({ sku, name, description = '', uom = 'unit', customFields = {}, reorderPoint = 0, reorderQty = 0, unitCost = 0 }) {
    if (!sku || !name) throw new Error('sku and name are required');
    const id = newId('item');
    try {
      db.prepare(`
        INSERT INTO inventory_items (id, sku, name, description, uom, custom_fields, reorder_point, reorder_qty, unit_cost)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, sku, name, description, uom, JSON.stringify(customFields), reorderPoint, reorderQty, unitCost);
    } catch (e) {
      throw new Error(e.message.includes('UNIQUE') ? `SKU ${sku} already exists` : e.message);
    }
    return { id, sku, name, description, uom, customFields, reorderPoint, reorderQty, unitCost };
  }

  function updateItem(id, { name, description, reorderPoint, reorderQty, active } = {}) {
