...
function getSession(sessionId) {
  ...
  const order = db.prepare('SELECT id, client_name, status, shipping FROM orders WHERE id = ?').get(s.order_id);
...
function openCarton(sessionId) {
  ...
  const existingOpen = getOpenCarton(sessionId);
  if (existingOpen) throw new Error('Close the current carton before opening a new one');

  // Generate unique HU: HU + date + daily global sequence
  const today = new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const { n: daySeq } = db.prepare(
    "SELECT COUNT(*) AS n FROM scan_cartons WHERE hu_code LIKE ?"
  ).get(`HU${today}%`);
...
function scanItem(sessionId, { sku, qty = 1, item_name = '', lot_number = '', expiry_date = '' }) {
  ...
  if (!item_name) {
    const inv = db.prepare('SELECT name FROM inventory WHERE sku = ?').get(sku);
    item_name = inv ? inv.name : sku;
  }
