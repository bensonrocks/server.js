...
(async()=>{
  const c=spawn('node',[require('path').join(__dirname,'../../server.js')],{env:{...process.env,PORT:String(PORT),DATA_DIR:DDIR},stdio:['ignore',fs.openSync(path.join(S,'bp-render.log'),'a'),fs.openSync(path.join(S,'bp-render.log'),'a')],detached:true});
  ...
  const pdfjsSrc=fs.readFileSync(require('path').join(__dirname,'../../node_modules/pdfjs-dist/legacy/build/pdf.js'),'utf8');
  const workerSrc=fs.readFileSync(require('path').join(__dirname,'../../node_modules/pdfjs-dist/legacy/build/pdf.worker.js'),'utf8');
