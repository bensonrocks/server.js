  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-24">
      ...
      <div className="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-2">
        {[featured1, featured2].map((service) => (
          <Link
            key={service.slug}
            href={`/services#${service.slug}`}
            className="group rounded-lg border border-border bg-paper p-8 transition-all hover:-translate-y-0.5 hover:border-brand hover:shadow-md"
          >
      ...
      <div className="mt-6 divide-y divide-border rounded-lg border border-border bg-paper">
        {rest.map((service) => (
          <Link
            key={service.slug}
            href={`/services#${service.slug}`}
